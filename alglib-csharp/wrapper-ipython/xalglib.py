...
if curdir=="":
    curdir = "."
_net_candidates = []
_net_candidates.append(os.path.join(os.path.relpath(curdir),"alglib404gpl_net2.dll"))
_net_candidates.append(os.path.join(sys.prefix,"alglib404gpl_net2.dll"))
_net_candidates.append(os.path.join(os.sep+"usr","local","alglib404gpl_net2.dll"))
...
#
# this function copies NET-vector to the previously allocated list 
# whose size must exactly match that of the x-vector.
#
# Parameters:
# * x       NET-vector
# * r       list which stores result
# * dt      datatype code
#
def net_to_list_inplace(x, dt, r):
    cnt = x.GetLength(0)
    if cnt!=len(r):
        raise ValueError("net_to_list_inplace(): sizes of x an r do not match")
    if dt==DT_COMPLEX:
        i = 0
        while i<cnt:
            r[i] = complex(x[i].x, x[i].y)
...
#
# this function copies NET-natrix to the previously allocated list of lists
# whose size must exactly match that of the x-matrix.
#
# Parameters:
# * x       NET-matrix
# * r       list which stores result
# * dt      datatype code
#
def net_to_listlist_inplace(x, dt, r):
    if x.GetLength(0)==0 or x.GetLength(1)==0:
        if len(r)!=0:
            raise ValueError("net_to_listlist_inplace(): sizes of x an r do not match")
        return
    if len(r)!=x.GetLength(0):
        raise ValueError("net_to_listlist_inplace(): sizes of x an r do not match")
    rows = x.GetLength(0)
    cols = x.GetLength(1)
    i=0
    while i<rows:
        ...
        j = 0
        while j<cols:
            r[i][j] = x[i,j] if dt!=DT_COMPLEX else complex(x[i,j].x, x[i,j].y)
            j += 1
        i += 1
    return
...
#
# Callback proxies which convert an IronPython function (working with arrays) to
# a C# delegate.
#
class _ipynet_proxy_func(object):
    def __init__(self, _func, _param):
        self.func  = _func
        self.param = _param
        
    def callback(self, x_net):
        x_py = list_from_net(x_net,DT_REAL)
        return self.func(x_py, self.param)
    
    def get_delegate(self):
        return _net_alglib._ipy_func_wrapper(self.callback).get_delegate()


class _ipynet_proxy_grad(object):
    def __init__(self, _grad, _param):
        self.grad  = _grad
        self.param = _param
        
    def callback(self, x_net, g_net):
        x_py = list_from_net(x_net,DT_REAL)
        g_py = list_from_net(g_net,DT_REAL)
        r = self.grad(x_py, g_py, self.param)
        copy_list_to_net(g_py, g_net, DT_REAL)
        return r
    
    def get_delegate(self):
        return _net_alglib._ipy_grad_wrapper(self.callback).get_delegate()


class _ipynet_proxy_pfunc(object):
    def __init__(self, _func, _param):
        self.func  = _func
        self.param = _param
        
    def callback(self, x_net, p_net):
        x_py = list_from_net(x_net,DT_REAL)
        p_py = list_from_net(p_net,DT_REAL)
        return self.func(x_py, p_py, self.param)
    
    def get_delegate(self):
        return _net_alglib._ipy_pfunc_wrapper(self.callback).get_delegate()


class _ipynet_proxy_pgrad(object):
    def __init__(self, _grad, _param):
        self.grad  = _grad
        self.param = _param
        
    def callback(self, x_net, p_net, g_net):
        x_py = list_from_net(x_net,DT_REAL)
        p_py = list_from_net(p_net,DT_REAL)
        g_py = list_from_net(g_net,DT_REAL)
        r = self.grad(x_py, p_py, g_py, self.param)
        copy_list_to_net(g_py, g_net, DT_REAL)
        return r
    
    def get_delegate(self):
        return _net_alglib._ipy_pgrad_wrapper(self.callback).get_delegate()


class _ipynet_proxy_fvec(object):
    def __init__(self, _fvec, _param):
        self.fvec  = _fvec
        self.param = _param
        
    def callback(self, x_net, f_net, dummy):
        x_py = list_from_net(x_net,DT_REAL)
        f_py = list_from_net(f_net,DT_REAL) # here we are mostly concerned with preallocating storage
        self.fvec(x_py, f_py, self.param)
        copy_list_to_net(f_py,f_net,DT_REAL)
    
    def get_delegate(self):
        return _net_alglib.ndimensional_fvec(self.callback)


class _ipynet_proxy_jac(object):
    def __init__(self, _jac, _param):
        self.jac  = _jac
        self.param = _param
        
    def callback(self, x_net, f_net, jac_net, dummy):
        x_py   = list_from_net(x_net,DT_REAL)
        f_py   = list_from_net(f_net,DT_REAL)       # here we are mostly concerned with preallocating storage
        jac_py = listlist_from_net(jac_net,DT_REAL) # here we are mostly concerned with preallocating storage
        self.jac(x_py, f_py, jac_py, self.param)
        copy_list_to_net(f_py,f_net,DT_REAL)
        copy_listlist_to_net(jac_py,jac_net,DT_REAL)
    
    def get_delegate(self):
        return _net_alglib.ndimensional_jac(self.callback)


class _ipynet_proxy_sjac(object):
    def __init__(self, _sjac, _param):
        self.sjac  = _sjac
        self.param = _param
    ...
    def callback(self, x_net, f_net, jac_net, dummy):
        x_py   = list_from_net(x_net,DT_REAL)
        f_py   = list_from_net(f_net,DT_REAL)       # here we are mostly concerned with preallocating storage
        jac_py = sparsematrix(jac_net) # here we are mostly concerned with preallocating storage
        self.sjac(x_py, f_py, jac_py, self.param)
        copy_list_to_net(f_py,f_net,DT_REAL)
    
    def get_delegate(self):
        return _net_alglib.ndimensional_sjac(self.callback)

class _ipynet_proxy_rep(object):
    def __init__(self, _rep, _param):
        self.rep   = _rep
        self.param = _rep
    ...
    def callback(self, x_net, f_net, dummy):
        ...
        if self.rep is not None:
            self.rep(x_py, f_py, self.param)
        return
    
    def get_delegate(self):
        return _net_alglib.ndimensional_rep(self.callback)
...
def net_to_xdebugrecord1_inplace(x,r):
    r.i = x.i
    r.c = complex(x.c.x,x.c.y)
    r.a = list_from_net(x.a, DT_REAL)
    return
...
def xdebugupdaterecord1(*functionargs):
    # unpack inputs
    rec1,  = functionargs
    friendly_form = False

    # convert to .NET types
    _net_rec1 = _net_alglib.xdebugrecord1()
    net_from_xdebugrecord1(_net_rec1, rec1)
    try:

        # call function
        _net_alglib.xdebugupdaterecord1(_net_rec1)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_xdebugrecord1_inplace(_net_rec1, rec1)
...
def xdebugb1not(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugb1not(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_a, DT_BOOL, a)
...
def xdebugi1neg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugi1neg(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_a, DT_INT, a)
...
def xdebugr1internalcopyandsum(*functionargs):
    # unpack inputs
    a,  = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.xdebugr1internalcopyandsum' must be real vector")
    try:

        # call function
        _net_result = _net_alglib.xdebugr1internalcopyandsum(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def xdebugr1neg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugr1neg(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_a, DT_REAL, a)
...
def xdebugc1neg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugc1neg(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_a, DT_COMPLEX, a)
...
def xdebugb2not(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugb2not(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_a, DT_BOOL, a)
...
def xdebugi2neg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugi2neg(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_a, DT_INT, a)
...
def xdebugr2internalcopyandsum(*functionargs):
    # unpack inputs
    a,  = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.xdebugr2internalcopyandsum' must be real matrix")
    try:

        # call function
        _net_result = _net_alglib.xdebugr2internalcopyandsum(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def xdebugr2neg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugr2neg(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_a, DT_REAL, a)
...
def xdebugc2neg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.xdebugc2neg(_net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_a, DT_COMPLEX, a)
...
def cmatrixtranspose(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixtranspose(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_b, _net_ib, _net_jb)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_b, DT_COMPLEX, b)
...
def rmatrixtranspose(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixtranspose(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_b, _net_ib, _net_jb)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_b, DT_REAL, b)
...
def rmatrixenforcesymmetricity(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixenforcesymmetricity(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixcopy(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixcopy(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_b, _net_ib, _net_jb)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rvectorcopy(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rvectorcopy(_net_n, _net_a, _net_ia, _net_b, _net_ib)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_b, DT_REAL, b)
...
def rmatrixcopy(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixcopy(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_b, _net_ib, _net_jb)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixgencopy(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixgencopy(_net_m, _net_n, _net_alpha, _net_a, _net_ia, _net_ja, _net_beta, _net_b, _net_ib, _net_jb)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixger(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixger(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_alpha, _net_u, _net_iu, _net_v, _net_iv)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixrank1(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixrank1(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_u, _net_iu, _net_v, _net_iv)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixrank1(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixrank1(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_u, _net_iu, _net_v, _net_iv)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixgemv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixgemv(_net_m, _net_n, _net_alpha, _net_a, _net_ia, _net_ja, _net_opa, _net_x, _net_ix, _net_beta, _net_y, _net_iy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_y, DT_REAL, y)
...
def cmatrixmv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixmv(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_opa, _net_x, _net_ix, _net_y, _net_iy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_y, DT_COMPLEX, y)
...
def rmatrixmv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixmv(_net_m, _net_n, _net_a, _net_ia, _net_ja, _net_opa, _net_x, _net_ix, _net_y, _net_iy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixsymv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixsymv(_net_n, _net_alpha, _net_a, _net_ia, _net_ja, _net_isupper, _net_x, _net_ix, _net_beta, _net_y, _net_iy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixsyvmv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixsyvmv(_net_n, _net_a, _net_ia, _net_ja, _net_isupper, _net_x, _net_ix, _net_tmp)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    result = _net_result
    result = _net_result
    net_to_list_inplace(_net_tmp, DT_REAL, tmp)
...
def rmatrixtrsv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixtrsv(_net_n, _net_a, _net_ia, _net_ja, _net_isupper, _net_isunit, _net_optype, _net_x, _net_ix)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_list_inplace(_net_x, DT_REAL, x)
...
def cmatrixrighttrsm(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixrighttrsm(_net_m, _net_n, _net_a, _net_i1, _net_j1, _net_isupper, _net_isunit, _net_optype, _net_x, _net_i2, _net_j2)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_x, DT_COMPLEX, x)
...
def cmatrixlefttrsm(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixlefttrsm(_net_m, _net_n, _net_a, _net_i1, _net_j1, _net_isupper, _net_isunit, _net_optype, _net_x, _net_i2, _net_j2)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixrighttrsm(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixrighttrsm(_net_m, _net_n, _net_a, _net_i1, _net_j1, _net_isupper, _net_isunit, _net_optype, _net_x, _net_i2, _net_j2)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_x, DT_REAL, x)
...
def rmatrixlefttrsm(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixlefttrsm(_net_m, _net_n, _net_a, _net_i1, _net_j1, _net_isupper, _net_isunit, _net_optype, _net_x, _net_i2, _net_j2)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixherk(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixherk(_net_n, _net_k, _net_alpha, _net_a, _net_ia, _net_ja, _net_optypea, _net_beta, _net_c, _net_ic, _net_jc, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_c, DT_COMPLEX, c)
...
def rmatrixsyrk(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixsyrk(_net_n, _net_k, _net_alpha, _net_a, _net_ia, _net_ja, _net_optypea, _net_beta, _net_c, _net_ic, _net_jc, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_c, DT_REAL, c)
...
def cmatrixgemm(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixgemm(_net_m, _net_n, _net_k, _net_alpha, _net_a, _net_ia, _net_ja, _net_optypea, _net_b, _net_ib, _net_jb, _net_optypeb, _net_beta, _net_c, _net_ic, _net_jc)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixgemm(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixgemm(_net_m, _net_n, _net_k, _net_alpha, _net_a, _net_ia, _net_ja, _net_optypea, _net_b, _net_ib, _net_jb, _net_optypeb, _net_beta, _net_c, _net_ic, _net_jc)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixsyrk(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixsyrk(_net_n, _net_k, _net_alpha, _net_a, _net_ia, _net_ja, _net_optypea, _net_beta, _net_c, _net_ic, _net_jc, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixqr(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau = _net_alglib.rmatrixqr(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixlq(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau = _net_alglib.rmatrixlq(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixqr(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau = _net_alglib.cmatrixqr(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixlq(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau = _net_alglib.cmatrixlq(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixbd(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tauq, _net_taup = _net_alglib.rmatrixbd(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixbdmultiplybyq(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixbdmultiplybyq(_net_qp, _net_m, _net_n, _net_tauq, _net_z, _net_zrows, _net_zcolumns, _net_fromtheright, _net_dotranspose)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_z, DT_REAL, z)
...
def rmatrixbdmultiplybyp(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixbdmultiplybyp(_net_qp, _net_m, _net_n, _net_taup, _net_z, _net_zrows, _net_zcolumns, _net_fromtheright, _net_dotranspose)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixhessenberg(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau = _net_alglib.rmatrixhessenberg(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def smatrixtd(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau, _net_d, _net_e = _net_alglib.smatrixtd(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hmatrixtd(*functionargs):
    ...
    try:
        ...
        # call function
        _net_tau, _net_d, _net_e = _net_alglib.hmatrixtd(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixrndorthogonalfromtheright(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixrndorthogonalfromtheright(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixrndorthogonalfromtheleft(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixrndorthogonalfromtheleft(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixrndorthogonalfromtheright(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixrndorthogonalfromtheright(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixrndorthogonalfromtheleft(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.cmatrixrndorthogonalfromtheleft(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def smatrixrndmultiply(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.smatrixrndmultiply(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hmatrixrndmultiply(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.hmatrixrndmultiply(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsecreatecrsempty(*functionargs):
    # unpack inputs
    n,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_s = _net_alglib.sparsecreatecrsempty(_net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsecreatecrsemptybuf(*functionargs):
    # unpack inputs
    n, s = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.sparsecreatecrsemptybuf(_net_n, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsecreatecrsfromdense(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a,  = functionargs
        m = check_equality_and_get([safe_rows("'sparsecreatecrsfromdense': incorrect parameters",a)],"Error while calling 'sparsecreatecrsfromdense': looks like one of arguments has wrong size")
        n = check_equality_and_get([safe_cols("'sparsecreatecrsfromdense': incorrect parameters",a)],"Error while calling 'sparsecreatecrsfromdense': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'sparsecreatecrsfromdense': function must have 3 or 1 parameters")

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.sparsecreatecrsfromdense' must be real matrix")
    _net_m = m
    _net_n = n
    try:

        # call function
        _net_s = _net_alglib.sparsecreatecrsfromdense(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsecreatecrsfromdensebuf(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, s = functionargs
        m = check_equality_and_get([safe_rows("'sparsecreatecrsfromdensebuf': incorrect parameters",a)],"Error while calling 'sparsecreatecrsfromdensebuf': looks like one of arguments has wrong size")
        n = check_equality_and_get([safe_cols("'sparsecreatecrsfromdensebuf': incorrect parameters",a)],"Error while calling 'sparsecreatecrsfromdensebuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'sparsecreatecrsfromdensebuf': function must have 4 or 2 parameters")

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.sparsecreatecrsfromdensebuf' must be real matrix")
    _net_m = m
    _net_n = n
    _net_s = s.ptr
    try:
        ...
        # call function
        _net_alglib.sparsecreatecrsfromdensebuf(_net_a, _net_m, _net_n, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsecreatecrsfromdensev(*functionargs):
    # unpack inputs
    a, m, n = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.sparsecreatecrsfromdensev' must be real vector")
    _net_m = m
    _net_n = n
    try:

        # call function
        _net_s = _net_alglib.sparsecreatecrsfromdensev(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsecreatecrsfromdensevbuf(*functionargs):
    # unpack inputs
    a, m, n, s = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.sparsecreatecrsfromdensevbuf' must be real vector")
    _net_m = m
    _net_n = n
    _net_s = s.ptr
    try:
        ...
        # call function
        _net_alglib.sparsecreatecrsfromdensevbuf(_net_a, _net_m, _net_n, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsegemv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.sparsegemv(_net_s, _net_alpha, _net_ops, _net_x, _net_ix, _net_beta, _net_y, _net_iy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsemultiplycolsby(*functionargs):
    # unpack inputs
    s, x = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.sparsemultiplycolsby' must be real vector")
    try:

        # call function
        _net_alglib.sparsemultiplycolsby(_net_s, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsemultiplyrowsby(*functionargs):
    # unpack inputs
    s, x = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.sparsemultiplyrowsby' must be real vector")
    try:

        # call function
        _net_alglib.sparsemultiplyrowsby(_net_s, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsetrmv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_y = _net_alglib.sparsetrmv(_net_s, _net_isupper, _net_isunit, _net_optype, _net_x, _net_y)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsetrsv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.sparsetrsv(_net_s, _net_isupper, _net_isunit, _net_optype, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsesymmpermtbltranspose(*functionargs):
    # unpack inputs
    a, isupper, p = functionargs
    friendly_form = False
    ...
    _net_a = a.ptr
    _net_isupper = isupper
    _net_p = net_from_list(p, DT_INT, "ALGLIB: parameter 'p' of 'xalglib.sparsesymmpermtbltranspose' must be int vector")
    try:

        # call function
        _net_b = _net_alglib.sparsesymmpermtbltranspose(_net_a, _net_isupper, _net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsesymmpermtbltransposebuf(*functionargs):
    # unpack inputs
    a, isupper, p, b = functionargs
    friendly_form = False
    ...
    _net_a = a.ptr
    _net_isupper = isupper
    _net_p = net_from_list(p, DT_INT, "ALGLIB: parameter 'p' of 'xalglib.sparsesymmpermtbltransposebuf' must be int vector")
    _net_b = b.ptr
    try:

        # call function
        _net_alglib.sparsesymmpermtbltransposebuf(_net_a, _net_isupper, _net_p, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparseappendcompressedrow(*functionargs):
    # unpack inputs
    s, colidx, vals, nz = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = s.ptr
    _net_colidx = net_from_list(colidx, DT_INT, "ALGLIB: parameter 'colidx' of 'xalglib.sparseappendcompressedrow' must be int vector")
    _net_vals = net_from_list(vals, DT_REAL, "ALGLIB: parameter 'vals' of 'xalglib.sparseappendcompressedrow' must be real vector")
    _net_nz = nz
    try:

        # call function
        _net_alglib.sparseappendcompressedrow(_net_s, _net_colidx, _net_vals, _net_nz)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparseappendemptyrow(*functionargs):
    # unpack inputs
    s,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.sparseappendemptyrow(_net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparseappendelement(*functionargs):
    # unpack inputs
    s, k, v = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.sparseappendelement(_net_s, _net_k, _net_v)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparseappendmatrix(*functionargs):
    # unpack inputs
    sdst, ssrc = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.sparseappendmatrix(_net_sdst, _net_ssrc)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsescale(*functionargs):
    # unpack inputs
    s, scltype, scalerows, scalecols, colsfirst = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = s.ptr
    _net_scltype = scltype
    _net_scalerows = scalerows
    _net_scalecols = scalecols
    _net_colsfirst = colsfirst
    try:

        # call function
        _net_r, _net_c = _net_alglib.sparsescale(_net_s, _net_scltype, _net_scalerows, _net_scalecols, _net_colsfirst)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_eigsubspacereport_inplace(x,r):
    r.iterationscount = x.iterationscount
    return
...
def smatrixtdevd(*functionargs):
    ...
    try:
        ...
        # call function
        _net_result, _net_z = _net_alglib.smatrixtdevd(_net_d, _net_e, _net_n, _net_zneeded, _net_z)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    result = _net_result
    result = _net_result
    net_to_list_inplace(_net_d, DT_REAL, d)
    z = listlist_from_net(_net_z, DT_REAL)
...
def rmatrixlu(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a,  = functionargs
        m = check_equality_and_get([safe_rows("'rmatrixlu': incorrect parameters",a)],"Error while calling 'rmatrixlu': looks like one of arguments has wrong size")
        n = check_equality_and_get([safe_cols("'rmatrixlu': incorrect parameters",a)],"Error while calling 'rmatrixlu': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixlu': function must have 3 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_pivots = _net_alglib.rmatrixlu(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixlu(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a,  = functionargs
        m = check_equality_and_get([safe_rows("'cmatrixlu': incorrect parameters",a)],"Error while calling 'cmatrixlu': looks like one of arguments has wrong size")
        n = check_equality_and_get([safe_cols("'cmatrixlu': incorrect parameters",a)],"Error while calling 'cmatrixlu': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixlu': function must have 3 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_pivots = _net_alglib.cmatrixlu(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixcholesky(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        n = check_equality_and_get([safe_rows("'hpdmatrixcholesky': incorrect parameters",a), safe_cols("'hpdmatrixcholesky': incorrect parameters",a)],"Error while calling 'hpdmatrixcholesky': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixcholesky': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.hpdmatrixcholesky(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholesky(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, isupper = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholesky': incorrect parameters",a), safe_cols("'spdmatrixcholesky': incorrect parameters",a)],"Error while calling 'spdmatrixcholesky': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholesky': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.spdmatrixcholesky(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskyupdateadd1(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, isupper, u = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholeskyupdateadd1': incorrect parameters",a), safe_cols("'spdmatrixcholeskyupdateadd1': incorrect parameters",a), safe_len("'spdmatrixcholeskyupdateadd1': incorrect parameters",u)],"Error while calling 'spdmatrixcholeskyupdateadd1': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholeskyupdateadd1': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.spdmatrixcholeskyupdateadd1(_net_a, _net_n, _net_isupper, _net_u)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskyupdatefix(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, isupper, fix = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholeskyupdatefix': incorrect parameters",a), safe_cols("'spdmatrixcholeskyupdatefix': incorrect parameters",a), safe_len("'spdmatrixcholeskyupdatefix': incorrect parameters",fix)],"Error while calling 'spdmatrixcholeskyupdatefix': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholeskyupdatefix': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.spdmatrixcholeskyupdatefix(_net_a, _net_n, _net_isupper, _net_fix)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskyupdateadd1buf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_bufr = _net_alglib.spdmatrixcholeskyupdateadd1buf(_net_a, _net_n, _net_isupper, _net_u, _net_bufr)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskyupdatefixbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_bufr = _net_alglib.spdmatrixcholeskyupdatefixbuf(_net_a, _net_n, _net_isupper, _net_fix, _net_bufr)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_polynomialsolverreport_inplace(x,r):
    r.maxerr = x.maxerr
    return
...
def rmatrixbdsvd(*functionargs):
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixbdsvd(_net_d, _net_e, _net_n, _net_isupper, _net_isfractionalaccuracyrequired, _net_u, _net_nru, _net_c, _net_ncc, _net_vt, _net_ncvt)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    result = _net_result
    net_to_list_inplace(_net_d, DT_REAL, d)
    net_to_listlist_inplace(_net_u, DT_REAL, u)
    net_to_listlist_inplace(_net_c, DT_REAL, c)
    net_to_listlist_inplace(_net_vt, DT_REAL, vt)
...
def rmatrixrcond2(*functionargs):
    # unpack inputs
    a, n = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.rmatrixrcond2' must be real matrix")
    _net_n = n
    try:

        # call function
        _net_result = _net_alglib.rmatrixrcond2(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixrcond2rect(*functionargs):
    # unpack inputs
    a, m, n = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.rmatrixrcond2rect' must be real matrix")
    _net_m = m
    _net_n = n
    try:

        # call function
        _net_result = _net_alglib.rmatrixrcond2rect(_net_a, _net_m, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixrcond2(*functionargs):
    # unpack inputs
    a, n, isupper = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.spdmatrixrcond2' must be real matrix")
    _net_n = n
    _net_isupper = isupper
    try:

        # call function
        _net_result = _net_alglib.spdmatrixrcond2(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixtrrcond2(*functionargs):
    # unpack inputs
    a, n, isupper, isunit = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.rmatrixtrrcond2' must be real matrix")
    _net_n = n
    _net_isupper = isupper
    _net_isunit = isunit
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixtrrcond2(_net_a, _net_n, _net_isupper, _net_isunit)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_densesolverreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.r1 = x.r1
    r.rinf = x.rinf
    return
...
def net_to_densesolverlsreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.r2 = x.r2
    r.cx = listlist_from_net(x.cx, DT_REAL)
    r.n = x.n
...
def rmatrixsolve(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixsolve': incorrect parameters",a), safe_cols("'rmatrixsolve': incorrect parameters",a), safe_len("'rmatrixsolve': incorrect parameters",b)],"Error while calling 'rmatrixsolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixsolve': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixsolve(_net_a, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixsolvefast(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixsolvefast': incorrect parameters",a), safe_cols("'rmatrixsolvefast': incorrect parameters",a), safe_len("'rmatrixsolvefast': incorrect parameters",b)],"Error while calling 'rmatrixsolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixsolvefast': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixsolvefast(_net_a, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixsolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, b, rfs = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixsolvem': incorrect parameters",a), safe_cols("'rmatrixsolvem': incorrect parameters",a), safe_rows("'rmatrixsolvem': incorrect parameters",b)],"Error while calling 'rmatrixsolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'rmatrixsolvem': incorrect parameters",b)],"Error while calling 'rmatrixsolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixsolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixsolvem(_net_a, _net_n, _net_b, _net_m, _net_rfs)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixsolvemfast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixsolvemfast': incorrect parameters",a), safe_cols("'rmatrixsolvemfast': incorrect parameters",a), safe_rows("'rmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'rmatrixsolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'rmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'rmatrixsolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixsolvemfast': function must have 4 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixsolvemfast(_net_a, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixlusolve(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixlusolve': incorrect parameters",lua), safe_cols("'rmatrixlusolve': incorrect parameters",lua), safe_len("'rmatrixlusolve': incorrect parameters",p), safe_len("'rmatrixlusolve': incorrect parameters",b)],"Error while calling 'rmatrixlusolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixlusolve': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixlusolve(_net_lua, _net_p, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixlusolvefast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixlusolvefast': incorrect parameters",lua), safe_cols("'rmatrixlusolvefast': incorrect parameters",lua), safe_len("'rmatrixlusolvefast': incorrect parameters",p)],"Error while calling 'rmatrixlusolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixlusolvefast': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixlusolvefast(_net_lua, _net_p, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixlusolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixlusolvem': incorrect parameters",lua), safe_cols("'rmatrixlusolvem': incorrect parameters",lua), safe_len("'rmatrixlusolvem': incorrect parameters",p), safe_rows("'rmatrixlusolvem': incorrect parameters",b)],"Error while calling 'rmatrixlusolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'rmatrixlusolvem': incorrect parameters",b)],"Error while calling 'rmatrixlusolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixlusolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixlusolvem(_net_lua, _net_p, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixlusolvemfast(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixlusolvemfast': incorrect parameters",lua), safe_cols("'rmatrixlusolvemfast': incorrect parameters",lua), safe_len("'rmatrixlusolvemfast': incorrect parameters",p), safe_rows("'rmatrixlusolvemfast': incorrect parameters",b)],"Error while calling 'rmatrixlusolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'rmatrixlusolvemfast': incorrect parameters",b)],"Error while calling 'rmatrixlusolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixlusolvemfast': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.rmatrixlusolvemfast(_net_lua, _net_p, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixmixedsolve(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixmixedsolve': incorrect parameters",a), safe_cols("'rmatrixmixedsolve': incorrect parameters",a), safe_rows("'rmatrixmixedsolve': incorrect parameters",lua), safe_cols("'rmatrixmixedsolve': incorrect parameters",lua), safe_len("'rmatrixmixedsolve': incorrect parameters",p), safe_len("'rmatrixmixedsolve': incorrect parameters",b)],"Error while calling 'rmatrixmixedsolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixmixedsolve': function must have 5 or 4 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixmixedsolve(_net_a, _net_lua, _net_p, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixmixedsolvem(*functionargs):
    ...
    if len(functionargs)==6:
        ...
        # short-form call
        a, lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'rmatrixmixedsolvem': incorrect parameters",a), safe_cols("'rmatrixmixedsolvem': incorrect parameters",a), safe_rows("'rmatrixmixedsolvem': incorrect parameters",lua), safe_cols("'rmatrixmixedsolvem': incorrect parameters",lua), safe_len("'rmatrixmixedsolvem': incorrect parameters",p), safe_rows("'rmatrixmixedsolvem': incorrect parameters",b)],"Error while calling 'rmatrixmixedsolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'rmatrixmixedsolvem': incorrect parameters",b)],"Error while calling 'rmatrixmixedsolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixmixedsolvem': function must have 6 or 4 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixmixedsolvem(_net_a, _net_lua, _net_p, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixsolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, b, rfs = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixsolvem': incorrect parameters",a), safe_cols("'cmatrixsolvem': incorrect parameters",a), safe_rows("'cmatrixsolvem': incorrect parameters",b)],"Error while calling 'cmatrixsolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'cmatrixsolvem': incorrect parameters",b)],"Error while calling 'cmatrixsolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixsolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.cmatrixsolvem(_net_a, _net_n, _net_b, _net_m, _net_rfs)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixsolvemfast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixsolvemfast': incorrect parameters",a), safe_cols("'cmatrixsolvemfast': incorrect parameters",a), safe_rows("'cmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'cmatrixsolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'cmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'cmatrixsolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixsolvemfast': function must have 4 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.cmatrixsolvemfast(_net_a, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixsolve(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixsolve': incorrect parameters",a), safe_cols("'cmatrixsolve': incorrect parameters",a), safe_len("'cmatrixsolve': incorrect parameters",b)],"Error while calling 'cmatrixsolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixsolve': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.cmatrixsolve(_net_a, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixsolvefast(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixsolvefast': incorrect parameters",a), safe_cols("'cmatrixsolvefast': incorrect parameters",a), safe_len("'cmatrixsolvefast': incorrect parameters",b)],"Error while calling 'cmatrixsolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixsolvefast': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.cmatrixsolvefast(_net_a, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    result = _net_result
    result = _net_result
    net_to_list_inplace(_net_b, DT_COMPLEX, b)
...
def cmatrixlusolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixlusolvem': incorrect parameters",lua), safe_cols("'cmatrixlusolvem': incorrect parameters",lua), safe_len("'cmatrixlusolvem': incorrect parameters",p), safe_rows("'cmatrixlusolvem': incorrect parameters",b)],"Error while calling 'cmatrixlusolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'cmatrixlusolvem': incorrect parameters",b)],"Error while calling 'cmatrixlusolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixlusolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.cmatrixlusolvem(_net_lua, _net_p, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixlusolvemfast(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixlusolvemfast': incorrect parameters",lua), safe_cols("'cmatrixlusolvemfast': incorrect parameters",lua), safe_len("'cmatrixlusolvemfast': incorrect parameters",p), safe_rows("'cmatrixlusolvemfast': incorrect parameters",b)],"Error while calling 'cmatrixlusolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'cmatrixlusolvemfast': incorrect parameters",b)],"Error while calling 'cmatrixlusolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixlusolvemfast': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.cmatrixlusolvemfast(_net_lua, _net_p, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixlusolve(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixlusolve': incorrect parameters",lua), safe_cols("'cmatrixlusolve': incorrect parameters",lua), safe_len("'cmatrixlusolve': incorrect parameters",p), safe_len("'cmatrixlusolve': incorrect parameters",b)],"Error while calling 'cmatrixlusolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixlusolve': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.cmatrixlusolve(_net_lua, _net_p, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixlusolvefast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixlusolvefast': incorrect parameters",lua), safe_cols("'cmatrixlusolvefast': incorrect parameters",lua), safe_len("'cmatrixlusolvefast': incorrect parameters",p)],"Error while calling 'cmatrixlusolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixlusolvefast': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.cmatrixlusolvefast(_net_lua, _net_p, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixmixedsolvem(*functionargs):
    ...
    if len(functionargs)==6:
        ...
        # short-form call
        a, lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixmixedsolvem': incorrect parameters",a), safe_cols("'cmatrixmixedsolvem': incorrect parameters",a), safe_rows("'cmatrixmixedsolvem': incorrect parameters",lua), safe_cols("'cmatrixmixedsolvem': incorrect parameters",lua), safe_len("'cmatrixmixedsolvem': incorrect parameters",p), safe_rows("'cmatrixmixedsolvem': incorrect parameters",b)],"Error while calling 'cmatrixmixedsolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'cmatrixmixedsolvem': incorrect parameters",b)],"Error while calling 'cmatrixmixedsolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixmixedsolvem': function must have 6 or 4 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.cmatrixmixedsolvem(_net_a, _net_lua, _net_p, _net_n, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixmixedsolve(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, lua, p, b = functionargs
        n = check_equality_and_get([safe_rows("'cmatrixmixedsolve': incorrect parameters",a), safe_cols("'cmatrixmixedsolve': incorrect parameters",a), safe_rows("'cmatrixmixedsolve': incorrect parameters",lua), safe_cols("'cmatrixmixedsolve': incorrect parameters",lua), safe_len("'cmatrixmixedsolve': incorrect parameters",p), safe_len("'cmatrixmixedsolve': incorrect parameters",b)],"Error while calling 'cmatrixmixedsolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'cmatrixmixedsolve': function must have 5 or 4 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.cmatrixmixedsolve(_net_a, _net_lua, _net_p, _net_n, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixsolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        n = check_equality_and_get([safe_rows("'spdmatrixsolvem': incorrect parameters",a), safe_cols("'spdmatrixsolvem': incorrect parameters",a), safe_rows("'spdmatrixsolvem': incorrect parameters",b)],"Error while calling 'spdmatrixsolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'spdmatrixsolvem': incorrect parameters",b)],"Error while calling 'spdmatrixsolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixsolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.spdmatrixsolvem(_net_a, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixsolvemfast(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixsolvemfast': incorrect parameters",a), safe_cols("'spdmatrixsolvemfast': incorrect parameters",a), safe_rows("'spdmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'spdmatrixsolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'spdmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'spdmatrixsolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixsolvemfast': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.spdmatrixsolvemfast(_net_a, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixsolve(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixsolve': incorrect parameters",a), safe_cols("'spdmatrixsolve': incorrect parameters",a), safe_len("'spdmatrixsolve': incorrect parameters",b)],"Error while calling 'spdmatrixsolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixsolve': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.spdmatrixsolve(_net_a, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixsolvefast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixsolvefast': incorrect parameters",a), safe_cols("'spdmatrixsolvefast': incorrect parameters",a), safe_len("'spdmatrixsolvefast': incorrect parameters",b)],"Error while calling 'spdmatrixsolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixsolvefast': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.spdmatrixsolvefast(_net_a, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskysolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholeskysolvem': incorrect parameters",cha), safe_cols("'spdmatrixcholeskysolvem': incorrect parameters",cha), safe_rows("'spdmatrixcholeskysolvem': incorrect parameters",b)],"Error while calling 'spdmatrixcholeskysolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'spdmatrixcholeskysolvem': incorrect parameters",b)],"Error while calling 'spdmatrixcholeskysolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholeskysolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.spdmatrixcholeskysolvem(_net_cha, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskysolvemfast(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholeskysolvemfast': incorrect parameters",cha), safe_cols("'spdmatrixcholeskysolvemfast': incorrect parameters",cha), safe_rows("'spdmatrixcholeskysolvemfast': incorrect parameters",b)],"Error while calling 'spdmatrixcholeskysolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'spdmatrixcholeskysolvemfast': incorrect parameters",b)],"Error while calling 'spdmatrixcholeskysolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholeskysolvemfast': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.spdmatrixcholeskysolvemfast(_net_cha, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskysolve(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholeskysolve': incorrect parameters",cha), safe_cols("'spdmatrixcholeskysolve': incorrect parameters",cha), safe_len("'spdmatrixcholeskysolve': incorrect parameters",b)],"Error while calling 'spdmatrixcholeskysolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholeskysolve': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.spdmatrixcholeskysolve(_net_cha, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskysolvefast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'spdmatrixcholeskysolvefast': incorrect parameters",cha), safe_cols("'spdmatrixcholeskysolvefast': incorrect parameters",cha), safe_len("'spdmatrixcholeskysolvefast': incorrect parameters",b)],"Error while calling 'spdmatrixcholeskysolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spdmatrixcholeskysolvefast': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.spdmatrixcholeskysolvefast(_net_cha, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixsolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixsolvem': incorrect parameters",a), safe_cols("'hpdmatrixsolvem': incorrect parameters",a), safe_rows("'hpdmatrixsolvem': incorrect parameters",b)],"Error while calling 'hpdmatrixsolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'hpdmatrixsolvem': incorrect parameters",b)],"Error while calling 'hpdmatrixsolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixsolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.hpdmatrixsolvem(_net_a, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixsolvemfast(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixsolvemfast': incorrect parameters",a), safe_cols("'hpdmatrixsolvemfast': incorrect parameters",a), safe_rows("'hpdmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'hpdmatrixsolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'hpdmatrixsolvemfast': incorrect parameters",b)],"Error while calling 'hpdmatrixsolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixsolvemfast': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.hpdmatrixsolvemfast(_net_a, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixsolve(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixsolve': incorrect parameters",a), safe_cols("'hpdmatrixsolve': incorrect parameters",a), safe_len("'hpdmatrixsolve': incorrect parameters",b)],"Error while calling 'hpdmatrixsolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixsolve': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.hpdmatrixsolve(_net_a, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixsolvefast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        a, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixsolvefast': incorrect parameters",a), safe_cols("'hpdmatrixsolvefast': incorrect parameters",a), safe_len("'hpdmatrixsolvefast': incorrect parameters",b)],"Error while calling 'hpdmatrixsolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixsolvefast': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.hpdmatrixsolvefast(_net_a, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixcholeskysolvem(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixcholeskysolvem': incorrect parameters",cha), safe_cols("'hpdmatrixcholeskysolvem': incorrect parameters",cha), safe_rows("'hpdmatrixcholeskysolvem': incorrect parameters",b)],"Error while calling 'hpdmatrixcholeskysolvem': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'hpdmatrixcholeskysolvem': incorrect parameters",b)],"Error while calling 'hpdmatrixcholeskysolvem': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixcholeskysolvem': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.hpdmatrixcholeskysolvem(_net_cha, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixcholeskysolvemfast(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixcholeskysolvemfast': incorrect parameters",cha), safe_cols("'hpdmatrixcholeskysolvemfast': incorrect parameters",cha), safe_rows("'hpdmatrixcholeskysolvemfast': incorrect parameters",b)],"Error while calling 'hpdmatrixcholeskysolvemfast': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_cols("'hpdmatrixcholeskysolvemfast': incorrect parameters",b)],"Error while calling 'hpdmatrixcholeskysolvemfast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixcholeskysolvemfast': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.hpdmatrixcholeskysolvemfast(_net_cha, _net_n, _net_isupper, _net_b, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixcholeskysolve(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixcholeskysolve': incorrect parameters",cha), safe_cols("'hpdmatrixcholeskysolve': incorrect parameters",cha), safe_len("'hpdmatrixcholeskysolve': incorrect parameters",b)],"Error while calling 'hpdmatrixcholeskysolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixcholeskysolve': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.hpdmatrixcholeskysolve(_net_cha, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixcholeskysolvefast(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        cha, isupper, b = functionargs
        n = check_equality_and_get([safe_rows("'hpdmatrixcholeskysolvefast': incorrect parameters",cha), safe_cols("'hpdmatrixcholeskysolvefast': incorrect parameters",cha), safe_len("'hpdmatrixcholeskysolvefast': incorrect parameters",b)],"Error while calling 'hpdmatrixcholeskysolvefast': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'hpdmatrixcholeskysolvefast': function must have 4 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.hpdmatrixcholeskysolvefast(_net_cha, _net_n, _net_isupper, _net_b)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixsolvels(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        a, b, threshold = functionargs
        nrows = check_equality_and_get([safe_rows("'rmatrixsolvels': incorrect parameters",a), safe_len("'rmatrixsolvels': incorrect parameters",b)],"Error while calling 'rmatrixsolvels': looks like one of arguments has wrong size")
        ncols = check_equality_and_get([safe_cols("'rmatrixsolvels': incorrect parameters",a)],"Error while calling 'rmatrixsolvels': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rmatrixsolvels': function must have 5 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.rmatrixsolvels(_net_a, _net_nrows, _net_ncols, _net_b, _net_threshold)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_sparsesolverreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.nmv = x.nmv
    r.iterationscount = x.iterationscount
    r.r2 = x.r2
...
def net_to_lincgreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nmv = x.nmv
    r.terminationtype = x.terminationtype
    r.r2 = x.r2
...
def net_to_linlsqrreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nmv = x.nmv
    r.terminationtype = x.terminationtype
    return
...
def sparsesolve(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, b = functionargs
        solvertype = check_equality_and_get([0],"Error while calling 'sparsesolve': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'sparsesolve': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_a = a.ptr
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.sparsesolve' must be real vector")
    _net_solvertype = solvertype
    try:

        # call function
        _net_x, _net_rep = _net_alglib.sparsesolve(_net_a, _net_b, _net_solvertype)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def sparsesolvelsreg(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        solvertype = check_equality_and_get([0],"Error while calling 'sparsesolvelsreg': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'sparsesolvelsreg': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_a = a.ptr
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.sparsesolvelsreg' must be real vector")
    _net_reg = reg
    _net_solvertype = solvertype
    try:

        # call function
        _net_x, _net_rep = _net_alglib.sparsesolvelsreg(_net_a, _net_b, _net_reg, _net_solvertype)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_nleqreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfunc = x.nfunc
    r.njac = x.njac
    r.terminationtype = x.terminationtype
...
def nleqsolve_fj(state, func, jac, rep = None, param = None):
    _pxy_func = _ipynet_proxy_func(func,param)
    _pxy_jac  = _ipynet_proxy_jac(jac,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.nleqsolve(state.ptr, _pxy_func.get_delegate(), _pxy_jac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def nleqresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.nleqresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_nleqreport_inplace(_net_rep, rep)
...
def net_to_optguardreport_inplace(x,r):
    r.nonc0suspected = x.nonc0suspected
    r.nonc0test0positive = x.nonc0test0positive
    r.nonc0fidx = x.nonc0fidx
    r.nonc0lipschitzc = x.nonc0lipschitzc
...
def net_from_optguardnonc0report(x,v):
    ...
    x.inneriter = int(v.inneriter)
    x.outeriter = int(v.outeriter)
    return
...
def optguardnonc0report_from_net(x):
    ...
    r.stpidxa = x.stpidxa
    r.stpidxb = x.stpidxb
    r.inneriter = x.inneriter
    r.outeriter = x.outeriter
    return r




def net_to_optguardnonc0report_inplace(x,r):
    r.positive = x.positive
    r.fidx = x.fidx
    r.x0 = list_from_net(x.x0, DT_REAL)
    r.d = list_from_net(x.d, DT_REAL)
...
def net_to_optguardnonc1test0report_inplace(x,r):
    r.positive = x.positive
    r.fidx = x.fidx
    r.x0 = list_from_net(x.x0, DT_REAL)
    r.d = list_from_net(x.d, DT_REAL)
...
def net_to_optguardnonc1test1report_inplace(x,r):
    r.positive = x.positive
    r.fidx = x.fidx
    r.vidx = x.vidx
    r.x0 = list_from_net(x.x0, DT_REAL)
...
def net_to_matinvreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.r1 = x.r1
    r.rinf = x.rinf
    return
...
def rmatrixluinverse(*functionargs):
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.rmatrixluinverse(_net_a, _net_pivots, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixinverse(*functionargs):
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.rmatrixinverse(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixluinverse(*functionargs):
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.cmatrixluinverse(_net_a, _net_pivots, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixinverse(*functionargs):
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.cmatrixinverse(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixcholeskyinverse(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        raise RuntimeError("Error while calling 'spdmatrixcholeskyinverse': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.spdmatrixcholeskyinverse(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixinverse(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        raise RuntimeError("Error while calling 'spdmatrixinverse': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.spdmatrixinverse(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixcholeskyinverse(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        raise RuntimeError("Error while calling 'hpdmatrixcholeskyinverse': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.hpdmatrixcholeskyinverse(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def hpdmatrixinverse(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        raise RuntimeError("Error while calling 'hpdmatrixinverse': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.hpdmatrixinverse(_net_a, _net_n, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixtrinverse(*functionargs):
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.rmatrixtrinverse(_net_a, _net_n, _net_isupper, _net_isunit)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def cmatrixtrinverse(*functionargs):
    ...
    try:
        ...
        # call function
        _net_rep = _net_alglib.cmatrixtrinverse(_net_a, _net_n, _net_isupper, _net_isunit)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_minlbfgsreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
    return
...
def minlbfgsoptimize_f(state, func, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minlbfgsoptimize(state.ptr, _pxy_func.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minlbfgsoptimize_g(state, grad, rep = None, param = None):
    _pxy_grad = _ipynet_proxy_grad(grad,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minlbfgsoptimize(state.ptr, _pxy_grad.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minlbfgsresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minlbfgsresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minlbfgsreport_inplace(_net_rep, rep)
...
def net_to_minlpreport_inplace(x,r):
    r.f = x.f
    r.lagbc = list_from_net(x.lagbc, DT_REAL)
    r.laglc = list_from_net(x.laglc, DT_REAL)
    r.y = list_from_net(x.y, DT_REAL)
...
def minlpsetalgoipm(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        state,  = functionargs
        eps = check_equality_and_get([0.0],"Error while calling 'minlpsetalgoipm': looks like one of arguments has wrong size")
        friendly_form = True
    else:
...
def minlpresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minlpresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minlpreport_inplace(_net_rep, rep)
...
def lptestproblemhasknowntarget(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.lptestproblemhasknowntarget(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lptestproblemgettargetf(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.lptestproblemgettargetf(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lptestproblemgetn(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.lptestproblemgetn(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lptestproblemgetm(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.lptestproblemgetm(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemcreate(*functionargs):
    # unpack inputs
    n,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_p = _net_alglib.qpxproblemcreate(_net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemisquadraticobjective(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemisquadraticobjective(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetn(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemgetn(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetmlc(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemgetmlc(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetmqc(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemgetmqc(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetmcc(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemgetmcc(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgettotalconstraints(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemgettotalconstraints(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemsetinitialpoint(*functionargs):
    # unpack inputs
    p, x0 = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_p = p.ptr
    _net_x0 = net_from_list(x0, DT_REAL, "ALGLIB: parameter 'x0' of 'xalglib.qpxproblemsetinitialpoint' must be real vector")
    try:

        # call function
        _net_alglib.qpxproblemsetinitialpoint(_net_p, _net_x0)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetinitialpoint(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_x0 = _net_alglib.qpxproblemgetinitialpoint(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x0 = list_from_net(_net_x0, DT_REAL)

    # return
    return x0

def qpxproblemhasinitialpoint(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemhasinitialpoint(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemsetscale(*functionargs):
    # unpack inputs
    p, s = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_p = p.ptr
    _net_s = net_from_list(s, DT_REAL, "ALGLIB: parameter 's' of 'xalglib.qpxproblemsetscale' must be real vector")
    try:

        # call function
        _net_alglib.qpxproblemsetscale(_net_p, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetscale(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_s = _net_alglib.qpxproblemgetscale(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    s = list_from_net(_net_s, DT_REAL)

    # return
    return s

def qpxproblemhasscale(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemhasscale(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemsetorigin(*functionargs):
    # unpack inputs
    p, xorigin = functionargs
    friendly_form = False

    # convert to .NET types
    _net_p = p.ptr
    _net_xorigin = net_from_list(xorigin, DT_REAL, "ALGLIB: parameter 'xorigin' of 'xalglib.qpxproblemsetorigin' must be real vector")
    try:

        # call function
        _net_alglib.qpxproblemsetorigin(_net_p, _net_xorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetorigin(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_xorigin = _net_alglib.qpxproblemgetorigin(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    xorigin = list_from_net(_net_xorigin, DT_REAL)

    # return
    return xorigin

def qpxproblemhasorigin(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemhasorigin(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemsetlinearterm(*functionargs):
    # unpack inputs
    p, c = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_p = p.ptr
    _net_c = net_from_list(c, DT_REAL, "ALGLIB: parameter 'c' of 'xalglib.qpxproblemsetlinearterm' must be real vector")
    try:

        # call function
        _net_alglib.qpxproblemsetlinearterm(_net_p, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetlinearterm(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_c = _net_alglib.qpxproblemgetlinearterm(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemsetquadraticterm(*functionargs):
    # unpack inputs
    p, q, isupper = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.qpxproblemsetquadraticterm(_net_p, _net_q, _net_isupper)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetquadraticterm(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_q, _net_isupper = _net_alglib.qpxproblemgetquadraticterm(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    q = sparsematrix(_net_q)
    isupper = _net_isupper

    # return
    return (q, isupper)

def qpxproblemhasquadraticterm(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.qpxproblemhasquadraticterm(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemsetbc(*functionargs):
    # unpack inputs
    p, bndl, bndu = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_p = p.ptr
    _net_bndl = net_from_list(bndl, DT_REAL, "ALGLIB: parameter 'bndl' of 'xalglib.qpxproblemsetbc' must be real vector")
    _net_bndu = net_from_list(bndu, DT_REAL, "ALGLIB: parameter 'bndu' of 'xalglib.qpxproblemsetbc' must be real vector")
    try:

        # call function
        _net_alglib.qpxproblemsetbc(_net_p, _net_bndl, _net_bndu)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetbc(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_bndl, _net_bndu = _net_alglib.qpxproblemgetbc(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    bndl = list_from_net(_net_bndl, DT_REAL)
    bndu = list_from_net(_net_bndu, DT_REAL)

    # return
    return (bndl, bndu)

def qpxproblemsetlc2(*functionargs):
    # unpack inputs
    p, a, al, au, m = functionargs
    friendly_form = False
    ...
    _net_p = p.ptr
    _net_a = a.ptr
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.qpxproblemsetlc2' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.qpxproblemsetlc2' must be real vector")
    _net_m = m
    try:

        # call function
        _net_alglib.qpxproblemsetlc2(_net_p, _net_a, _net_al, _net_au, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetlc2(*functionargs):
    # unpack inputs
    p,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_a, _net_al, _net_au, _net_m = _net_alglib.qpxproblemgetlc2(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    a = sparsematrix(_net_a)
    al = list_from_net(_net_al, DT_REAL)
    au = list_from_net(_net_au, DT_REAL)
    m = _net_m

    # return
    return (a, al, au, m)

def qpxproblemaddqc2(*functionargs):
    # unpack inputs
    p, q, isupper, b, cl, cu, applyorigin = functionargs
    friendly_form = False
    ...
    _net_q = q.ptr
    _net_isupper = isupper
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.qpxproblemaddqc2' must be real vector")
    _net_cl = cl
    _net_cu = cu
    _net_applyorigin = applyorigin
    try:

        # call function
        _net_alglib.qpxproblemaddqc2(_net_p, _net_q, _net_isupper, _net_b, _net_cl, _net_cu, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def qpxproblemgetqc2i(*functionargs):
    # unpack inputs
    p, idx = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_q, _net_isupper, _net_b, _net_cl, _net_cu, _net_applyorigin = _net_alglib.qpxproblemgetqc2i(_net_p, _net_idx)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    cl = _net_cl
    cu = _net_cu
    applyorigin = _net_applyorigin

    # return
    return (q, isupper, b, cl, cu, applyorigin)
...
def net_from_minqpreport(x,v):
    ...
    x.lagbc = net_from_list(v.lagbc, DT_REAL)
    x.laglc = net_from_list(v.laglc, DT_REAL)
    x.lagqc = net_from_list(v.lagqc, DT_REAL)
    return
...
def minqpreport_from_net(x):
    ...
    r.lagbc = list_from_net(x.lagbc, DT_REAL)
    r.laglc = list_from_net(x.laglc, DT_REAL)
    r.lagqc = list_from_net(x.lagqc, DT_REAL)
    return r




def net_to_minqpreport_inplace(x,r):
    r.inneriterationscount = x.inneriterationscount
    r.outeriterationscount = x.outeriterationscount
    r.nmv = x.nmv
    r.ncholesky = x.ncholesky
...
def minqpsetalgodensegenipm(*functionargs):
    # unpack inputs
    state, eps = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minqpsetalgodensegenipm(_net_state, _net_eps)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpsetalgosparsegenipm(*functionargs):
    # unpack inputs
    state, eps = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minqpsetalgosparsegenipm(_net_state, _net_eps)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpsetalgosparseecqp(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minqpsetalgosparseecqp(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpclearqc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.minqpclearqc(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddqc2(*functionargs):
    # unpack inputs
    state, q, isupper, b, cl, cu, applyorigin = functionargs
    friendly_form = False
    ...
    _net_q = q.ptr
    _net_isupper = isupper
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.minqpaddqc2' must be real vector")
    _net_cl = cl
    _net_cu = cu
    _net_applyorigin = applyorigin
    try:
        ...
        # call function
        _net_result = _net_alglib.minqpaddqc2(_net_state, _net_q, _net_isupper, _net_b, _net_cl, _net_cu, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddqc2list(*functionargs):
    # unpack inputs
    state, qridx, qcidx, qvals, qnnz, isupper, bidx, bvals, bnnz, cl, cu, applyorigin = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_qridx = net_from_list(qridx, DT_INT, "ALGLIB: parameter 'qridx' of 'xalglib.minqpaddqc2list' must be int vector")
    _net_qcidx = net_from_list(qcidx, DT_INT, "ALGLIB: parameter 'qcidx' of 'xalglib.minqpaddqc2list' must be int vector")
    _net_qvals = net_from_list(qvals, DT_REAL, "ALGLIB: parameter 'qvals' of 'xalglib.minqpaddqc2list' must be real vector")
    _net_qnnz = qnnz
    _net_isupper = isupper
    _net_bidx = net_from_list(bidx, DT_INT, "ALGLIB: parameter 'bidx' of 'xalglib.minqpaddqc2list' must be int vector")
    _net_bvals = net_from_list(bvals, DT_REAL, "ALGLIB: parameter 'bvals' of 'xalglib.minqpaddqc2list' must be real vector")
    _net_bnnz = bnnz
    _net_cl = cl
    _net_cu = cu
    _net_applyorigin = applyorigin
    try:
        ...
        # call function
        _net_result = _net_alglib.minqpaddqc2list(_net_state, _net_qridx, _net_qcidx, _net_qvals, _net_qnnz, _net_isupper, _net_bidx, _net_bvals, _net_bnnz, _net_cl, _net_cu, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddqc2dense(*functionargs):
    # unpack inputs
    state, q, isupper, b, cl, cu, applyorigin = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_q = net_from_listlist(q, DT_REAL, "ALGLIB: parameter 'q' of 'xalglib.minqpaddqc2dense' must be real matrix")
    _net_isupper = isupper
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.minqpaddqc2dense' must be real vector")
    _net_cl = cl
    _net_cu = cu
    _net_applyorigin = applyorigin
    try:
        ...
        # call function
        _net_result = _net_alglib.minqpaddqc2dense(_net_state, _net_q, _net_isupper, _net_b, _net_cl, _net_cu, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpclearcc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.minqpclearcc(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddsoccprimitive(*functionargs):
    # unpack inputs
    state, range0, range1, axisidx, applyorigin = functionargs
    friendly_form = False
    ...
    _net_range0 = range0
    _net_range1 = range1
    _net_axisidx = axisidx
    _net_applyorigin = applyorigin
    try:

        # call function
        _net_result = _net_alglib.minqpaddsoccprimitive(_net_state, _net_range0, _net_range1, _net_axisidx, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddsoccorthogonal(*functionargs):
    # unpack inputs
    state, idx, a, c, k, theta, applyorigin = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_idx = net_from_list(idx, DT_INT, "ALGLIB: parameter 'idx' of 'xalglib.minqpaddsoccorthogonal' must be int vector")
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.minqpaddsoccorthogonal' must be real vector")
    _net_c = net_from_list(c, DT_REAL, "ALGLIB: parameter 'c' of 'xalglib.minqpaddsoccorthogonal' must be real vector")
    _net_k = k
    _net_theta = theta
    _net_applyorigin = applyorigin
    try:
        ...
        # call function
        _net_result = _net_alglib.minqpaddsoccorthogonal(_net_state, _net_idx, _net_a, _net_c, _net_k, _net_theta, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddpowccprimitive(*functionargs):
    # unpack inputs
    state, range0, range1, axisidx, alpha, applyorigin = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.minqpaddpowccprimitive(_net_state, _net_range0, _net_range1, _net_axisidx, _net_alpha, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpaddpowccorthogonal(*functionargs):
    # unpack inputs
    state, idx, a, c, k, theta, alphav, kpow, applyorigin = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_idx = net_from_list(idx, DT_INT, "ALGLIB: parameter 'idx' of 'xalglib.minqpaddpowccorthogonal' must be int vector")
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.minqpaddpowccorthogonal' must be real vector")
    _net_c = net_from_list(c, DT_REAL, "ALGLIB: parameter 'c' of 'xalglib.minqpaddpowccorthogonal' must be real vector")
    _net_k = k
    _net_theta = theta
    _net_alphav = net_from_list(alphav, DT_REAL, "ALGLIB: parameter 'alphav' of 'xalglib.minqpaddpowccorthogonal' must be real vector")
    _net_kpow = kpow
    _net_applyorigin = applyorigin
    try:

        # call function
        _net_result = _net_alglib.minqpaddpowccorthogonal(_net_state, _net_idx, _net_a, _net_c, _net_k, _net_theta, _net_alphav, _net_kpow, _net_applyorigin)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minqpresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minqpreport_inplace(_net_rep, rep)

    # return
    return x

def minqpexport(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_p = _net_alglib.minqpexport(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minqpimport(*functionargs):
    ...
    try:
        ...
        # call function
        _net_s = _net_alglib.minqpimport(_net_p)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_minlmreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.terminationtype = x.terminationtype
    r.f = x.f
    r.nfunc = x.nfunc
...
def minlmsetnonmonotonicsteps(*functionargs):
    # unpack inputs
    state, cnt = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minlmsetnonmonotonicsteps(_net_state, _net_cnt)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minlmsetnumdiff(*functionargs):
    # unpack inputs
    state, formulatype = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_formulatype = formulatype
    try:

        # call function
        _net_alglib.minlmsetnumdiff(_net_state, _net_formulatype)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minlmoptimize_v(state, fvec, rep = None, param = None):
    _pxy_fvec = _ipynet_proxy_fvec(fvec,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minlmoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minlmoptimize_vj(state, fvec, jac, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minlmoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_jac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minlmresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minlmresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minlmreport_inplace(_net_rep, rep)
...
def net_to_mincgreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
    return
...
def mincgoptimize_f(state, func, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.mincgoptimize(state.ptr, _pxy_func.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def mincgoptimize_g(state, grad, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.mincgoptimize(state.ptr, _pxy_grad.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def mincgresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.mincgresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_mincgreport_inplace(_net_rep, rep)
...
def net_from_mindfreport(x,v):
    ...
    x.terminationtype = int(v.terminationtype)
    x.timetotal = float(v.timetotal)
    x.timesolver = float(v.timesolver)
    x.timecallback = float(v.timecallback)
    return




def mindfreport_from_net(x):
    r = mindfreport()
    r.f = x.f
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    ...
    r.nlcerr = x.nlcerr
    r.terminationtype = x.terminationtype
    r.timetotal = x.timetotal
    r.timesolver = x.timesolver
    r.timecallback = x.timecallback
    return r




def net_to_mindfreport_inplace(x,r):
    r.f = x.f
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.bcerr = x.bcerr
...
def mindfcreate(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        x,  = functionargs
        n = check_equality_and_get([safe_len("'mindfcreate': incorrect parameters",x)],"Error while calling 'mindfcreate': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'mindfcreate': function must have 2 or 1 parameters")

    # convert to .NET types
    _net_n = n
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.mindfcreate' must be real vector")
    try:

        # call function
        _net_state = _net_alglib.mindfcreate(_net_n, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    state = mindfstate(_net_state)

    # return
    return state

def mindfsetbc(*functionargs):
    # unpack inputs
    state, bndl, bndu = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_bndl = net_from_list(bndl, DT_REAL, "ALGLIB: parameter 'bndl' of 'xalglib.mindfsetbc' must be real vector")
    _net_bndu = net_from_list(bndu, DT_REAL, "ALGLIB: parameter 'bndu' of 'xalglib.mindfsetbc' must be real vector")
    try:

        # call function
        _net_alglib.mindfsetbc(_net_state, _net_bndl, _net_bndu)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetlc2dense(*functionargs):
    ...
    if len(functionargs)==5:
        ...
        # short-form call
        state, a, al, au = functionargs
        k = check_equality_and_get([safe_rows("'mindfsetlc2dense': incorrect parameters",a), safe_len("'mindfsetlc2dense': incorrect parameters",al), safe_len("'mindfsetlc2dense': incorrect parameters",au)],"Error while calling 'mindfsetlc2dense': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'mindfsetlc2dense': function must have 5 or 4 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.mindfsetlc2dense' must be real matrix")
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.mindfsetlc2dense' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.mindfsetlc2dense' must be real vector")
    _net_k = k
    try:

        # call function
        _net_alglib.mindfsetlc2dense(_net_state, _net_a, _net_al, _net_au, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetnlc2(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        nnlc = check_equality_and_get([safe_len("'mindfsetnlc2': incorrect parameters",nl), safe_len("'mindfsetnlc2': incorrect parameters",nu)],"Error while calling 'mindfsetnlc2': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'mindfsetnlc2': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_nl = net_from_list(nl, DT_REAL, "ALGLIB: parameter 'nl' of 'xalglib.mindfsetnlc2' must be real vector")
    _net_nu = net_from_list(nu, DT_REAL, "ALGLIB: parameter 'nu' of 'xalglib.mindfsetnlc2' must be real vector")
    _net_nnlc = nnlc
    try:

        # call function
        _net_alglib.mindfsetnlc2(_net_state, _net_nl, _net_nu, _net_nnlc)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetcondfx(*functionargs):
    # unpack inputs
    state, epsf, epsx = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetcondfx(_net_state, _net_epsf, _net_epsx)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfusetimers(*functionargs):
    # unpack inputs
    state, usetimers = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_usetimers = usetimers
    try:

        # call function
        _net_alglib.mindfusetimers(_net_state, _net_usetimers)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetcondf(*functionargs):
    # unpack inputs
    state, epsf = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetcondf(_net_state, _net_epsf)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetscale(*functionargs):
    # unpack inputs
    state, s = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_s = net_from_list(s, DT_REAL, "ALGLIB: parameter 's' of 'xalglib.mindfsetscale' must be real vector")
    try:

        # call function
        _net_alglib.mindfsetscale(_net_state, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetxrep(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetxrep(_net_state, _net_needxrep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfrequesttermination(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.mindfrequesttermination(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetseed(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetseed(_net_s, _net_seedval)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetalgogdemo(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        popsize = check_equality_and_get([0],"Error while calling 'mindfsetalgogdemo': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'mindfsetalgogdemo': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_epochscnt = epochscnt
    _net_popsize = popsize
    try:

        # call function
        _net_alglib.mindfsetalgogdemo(_net_state, _net_epochscnt, _net_popsize)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetgdemoprofilerobust(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetgdemoprofilerobust(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetgdemoprofilequick(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetgdemoprofilequick(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetgdemopenalty(*functionargs):
    # unpack inputs
    state, rho1, rho2 = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.mindfsetgdemopenalty(_net_state, _net_rho1, _net_rho2)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfsetalgogdemofixed(*functionargs):
    # unpack inputs
    state, epochscnt, strategy, crossoverprob, differentialweight, popsize = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_epochscnt = epochscnt
    _net_strategy = strategy
    _net_crossoverprob = crossoverprob
    _net_differentialweight = differentialweight
    _net_popsize = popsize
    try:

        # call function
        _net_alglib.mindfsetalgogdemofixed(_net_state, _net_epochscnt, _net_strategy, _net_crossoverprob, _net_differentialweight, _net_popsize)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def mindfoptimize_v(state, fvec, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.mindfoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return


def mindfresults(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.mindfresults(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    rep = mindfreport_from_net(_net_rep)

    # return
    return (x, rep)

def mindfresultsbuf(*functionargs):
    # unpack inputs
    state, x, rep = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.mindfresultsbuf' must be real vector")
    _net_rep = _net_alglib.mindfreport()
    net_from_mindfreport(_net_rep, rep)
    try:

        # call function
        _net_x = _net_alglib.mindfresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_mindfreport_inplace(_net_rep, rep)
...
def net_to_nlsreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.terminationtype = x.terminationtype
    r.nfunc = x.nfunc
    return
...
def nlscreatedfo(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        m, x = functionargs
        n = check_equality_and_get([safe_len("'nlscreatedfo': incorrect parameters",x)],"Error while calling 'nlscreatedfo': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'nlscreatedfo': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_n = n
    _net_m = m
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.nlscreatedfo' must be real vector")
    try:

        # call function
        _net_state = _net_alglib.nlscreatedfo(_net_n, _net_m, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    state = nlsstate(_net_state)
...
def nlssetalgo2ps(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        state,  = functionargs
        nnoisyrestarts = check_equality_and_get([0],"Error while calling 'nlssetalgo2ps': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'nlssetalgo2ps': function must have 2 or 1 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_nnoisyrestarts = nnoisyrestarts
    try:

        # call function
        _net_alglib.nlssetalgo2ps(_net_state, _net_nnoisyrestarts)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlssetalgodfolsa(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        state,  = functionargs
        nnoisyrestarts = check_equality_and_get([0],"Error while calling 'nlssetalgodfolsa': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'nlssetalgodfolsa': function must have 2 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.nlssetalgodfolsa(_net_state, _net_nnoisyrestarts)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlssetcond(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.nlssetcond(_net_state, _net_epsx, _net_maxits)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlssetxrep(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.nlssetxrep(_net_state, _net_needxrep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlssetscale(*functionargs):
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_s = net_from_list(s, DT_REAL, "ALGLIB: parameter 's' of 'xalglib.nlssetscale' must be real vector")
    try:

        # call function
        _net_alglib.nlssetscale(_net_state, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlssetbc(*functionargs):
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_bndl = net_from_list(bndl, DT_REAL, "ALGLIB: parameter 'bndl' of 'xalglib.nlssetbc' must be real vector")
    _net_bndu = net_from_list(bndu, DT_REAL, "ALGLIB: parameter 'bndu' of 'xalglib.nlssetbc' must be real vector")
    try:

        # call function
        _net_alglib.nlssetbc(_net_state, _net_bndl, _net_bndu)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlsoptimize_v(state, fvec, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.nlsoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def nlsoptimize_vj(state, fvec, jac, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.nlsoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_jac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return


def nlsresults(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_x, _net_rep = _net_alglib.nlsresults(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    rep = nlsreport_from_net(_net_rep)

    # return
    return (x, rep)

def nlsresultsbuf(*functionargs):
    # unpack inputs
    state, x, rep = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.nlsresultsbuf' must be real vector")
    _net_rep = _net_alglib.nlsreport()
    net_from_nlsreport(_net_rep, rep)
    try:

        # call function
        _net_x = _net_alglib.nlsresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_nlsreport_inplace(_net_rep, rep)

    # return
    return x

def nlsrestartfrom(*functionargs):
    # unpack inputs
    state, x = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.nlsrestartfrom' must be real vector")
    try:

        # call function
        _net_alglib.nlsrestartfrom(_net_state, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def nlsrequesttermination(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.nlsrequesttermination(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_minbleicreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.varidx = x.varidx
    r.terminationtype = x.terminationtype
...
def minbleicoptimize_f(state, func, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minbleicoptimize(state.ptr, _pxy_func.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minbleicoptimize_g(state, grad, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minbleicoptimize(state.ptr, _pxy_grad.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minbleicresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minbleicresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minbleicreport_inplace(_net_rep, rep)
...
def net_from_minnlcreport(x,v):
    ...
    x.lagbc = net_from_list(v.lagbc, DT_REAL)
    x.lagbcnz = net_from_list(v.lagbcnz, DT_REAL)
    x.laglc = net_from_list(v.laglc, DT_REAL)
    x.lagnlc = net_from_list(v.lagnlc, DT_REAL)
...
def minnlcreport_from_net(x):
    ...
    r.nlcidx = x.nlcidx
    r.lagbc = list_from_net(x.lagbc, DT_REAL)
    r.lagbcnz = list_from_net(x.lagbcnz, DT_REAL)
    r.laglc = list_from_net(x.laglc, DT_REAL)
    r.lagnlc = list_from_net(x.lagnlc, DT_REAL)
...
def net_to_minnlcreport_inplace(x,r):
    r.f = x.f
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
...
def minnlccreatebuf(*functionargs):
    # unpack inputs
    if len(functionargs)==3:
        # full-form call
        n, x, state = functionargs
        friendly_form = False
    elif len(functionargs)==2:
        # short-form call
        x, state = functionargs
        n = check_equality_and_get([safe_len("'minnlccreatebuf': incorrect parameters",x)],"Error while calling 'minnlccreatebuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minnlccreatebuf': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_n = n
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.minnlccreatebuf' must be real vector")
    _net_state = state.ptr
    try:

        # call function
        _net_alglib.minnlccreatebuf(_net_n, _net_x, _net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlccreatefbuf(*functionargs):
    # unpack inputs
    if len(functionargs)==4:
        # full-form call
        n, x, diffstep, state = functionargs
        friendly_form = False
    elif len(functionargs)==3:
        # short-form call
        x, diffstep, state = functionargs
        n = check_equality_and_get([safe_len("'minnlccreatefbuf': incorrect parameters",x)],"Error while calling 'minnlccreatefbuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minnlccreatefbuf': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_n = n
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.minnlccreatefbuf' must be real vector")
    _net_diffstep = diffstep
    _net_state = state.ptr
    try:

        # call function
        _net_alglib.minnlccreatefbuf(_net_n, _net_x, _net_diffstep, _net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetlc2dense(*functionargs):
    # unpack inputs
    if len(functionargs)==5:
        # full-form call
        state, a, al, au, k = functionargs
        ...
        # short-form call
        state, a, al, au = functionargs
        k = check_equality_and_get([safe_rows("'minnlcsetlc2dense': incorrect parameters",a), safe_len("'minnlcsetlc2dense': incorrect parameters",al), safe_len("'minnlcsetlc2dense': incorrect parameters",au)],"Error while calling 'minnlcsetlc2dense': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minnlcsetlc2dense': function must have 5 or 4 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.minnlcsetlc2dense' must be real matrix")
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.minnlcsetlc2dense' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.minnlcsetlc2dense' must be real vector")
    _net_k = k
    try:

        # call function
        _net_alglib.minnlcsetlc2dense(_net_state, _net_a, _net_al, _net_au, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetlc2(*functionargs):
    ...
    _net_state = state.ptr
    _net_a = a.ptr
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.minnlcsetlc2' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.minnlcsetlc2' must be real vector")
    _net_k = k
    try:

        # call function
        _net_alglib.minnlcsetlc2(_net_state, _net_a, _net_al, _net_au, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetlc2mixed(*functionargs):
    # unpack inputs
    state, sparsea, ksparse, densea, kdense, al, au = functionargs
    friendly_form = False
    ...
    _net_sparsea = sparsea.ptr
    _net_ksparse = ksparse
    _net_densea = net_from_listlist(densea, DT_REAL, "ALGLIB: parameter 'densea' of 'xalglib.minnlcsetlc2mixed' must be real matrix")
    _net_kdense = kdense
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.minnlcsetlc2mixed' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.minnlcsetlc2mixed' must be real vector")
    try:

        # call function
        _net_alglib.minnlcsetlc2mixed(_net_state, _net_sparsea, _net_ksparse, _net_densea, _net_kdense, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcaddlc2dense(*functionargs):
    # unpack inputs
    state, a, al, au = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.minnlcaddlc2dense' must be real vector")
    _net_al = al
    _net_au = au
    try:

        # call function
        _net_alglib.minnlcaddlc2dense(_net_state, _net_a, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcaddlc2(*functionargs):
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_idxa = net_from_list(idxa, DT_INT, "ALGLIB: parameter 'idxa' of 'xalglib.minnlcaddlc2' must be int vector")
    _net_vala = net_from_list(vala, DT_REAL, "ALGLIB: parameter 'vala' of 'xalglib.minnlcaddlc2' must be real vector")
    _net_nnz = nnz
    _net_al = al
    _net_au = au
    try:
        ...
        # call function
        _net_alglib.minnlcaddlc2(_net_state, _net_idxa, _net_vala, _net_nnz, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcaddlc2sparsefromdense(*functionargs):
    # unpack inputs
    state, da, al, au = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_da = net_from_list(da, DT_REAL, "ALGLIB: parameter 'da' of 'xalglib.minnlcaddlc2sparsefromdense' must be real vector")
    _net_al = al
    _net_au = au
    try:

        # call function
        _net_alglib.minnlcaddlc2sparsefromdense(_net_state, _net_da, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetnlc2(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        state, nl, nu = functionargs
        nnlc = check_equality_and_get([safe_len("'minnlcsetnlc2': incorrect parameters",nl), safe_len("'minnlcsetnlc2': incorrect parameters",nu)],"Error while calling 'minnlcsetnlc2': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minnlcsetnlc2': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_nl = net_from_list(nl, DT_REAL, "ALGLIB: parameter 'nl' of 'xalglib.minnlcsetnlc2' must be real vector")
    _net_nu = net_from_list(nu, DT_REAL, "ALGLIB: parameter 'nu' of 'xalglib.minnlcsetnlc2' must be real vector")
    _net_nnlc = nnlc
    try:

        # call function
        _net_alglib.minnlcsetnlc2(_net_state, _net_nl, _net_nu, _net_nnlc)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetnumdiff(*functionargs):
    # unpack inputs
    state, formulatype = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minnlcsetnumdiff(_net_state, _net_formulatype)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetcond3(*functionargs):
    # unpack inputs
    ...
    try:
        ...
        # call function
        _net_alglib.minnlcsetcond3(_net_state, _net_epsf, _net_epsx, _net_maxits)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetalgoaul2(*functionargs):
    # unpack inputs
    state, maxouterits = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minnlcsetalgoaul2(_net_state, _net_maxouterits)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetalgosl1qp(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minnlcsetalgosl1qp(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetalgosl1qpbfgs(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minnlcsetalgosl1qpbfgs(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetalgoorbit(*functionargs):
    # unpack inputs
    state, rad0, maxnfev = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_rad0 = rad0
    _net_maxnfev = maxnfev
    try:

        # call function
        _net_alglib.minnlcsetalgoorbit(_net_state, _net_rad0, _net_maxnfev)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcsetalgosqpbfgs(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minnlcsetalgosqpbfgs(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minnlcoptimize_v(state, fvec, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minnlcoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minnlcoptimize_j(state, jac, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minnlcoptimize(state.ptr, _pxy_jac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return


def minnlcoptimize_s(state, sjac, rep = None, param = None):
    _pxy_sjac  = _ipynet_proxy_sjac(sjac,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minnlcoptimize(state.ptr, _pxy_sjac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minnlcresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minnlcresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minnlcreport_inplace(_net_rep, rep)
...
class minmostate(object):
    def __init__(self,ptr):
        self.ptr = ptr


class minmoreport(object):
    def __init__(self):
        self.inneriterationscount = 0
        self.outeriterationscount = 0
        self.nfev = 0
...
def net_to_minmoreport_inplace(x,r):
    r.inneriterationscount = x.inneriterationscount
    r.outeriterationscount = x.outeriterationscount
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
...
def minmocreate(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        m, x = functionargs
        n = check_equality_and_get([safe_len("'minmocreate': incorrect parameters",x)],"Error while calling 'minmocreate': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minmocreate': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_n = n
    _net_m = m
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.minmocreate' must be real vector")
    try:

        # call function
        _net_state = _net_alglib.minmocreate(_net_n, _net_m, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    state = minmostate(_net_state)

    # return
    return state

def minmocreatef(*functionargs):
    # unpack inputs
    if len(functionargs)==4:
        # full-form call
        n, m, x, diffstep = functionargs
        ...
        # short-form call
        m, x, diffstep = functionargs
        n = check_equality_and_get([safe_len("'minmocreatef': incorrect parameters",x)],"Error while calling 'minmocreatef': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minmocreatef': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_n = n
    _net_m = m
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.minmocreatef' must be real vector")
    _net_diffstep = diffstep
    try:

        # call function
        _net_state = _net_alglib.minmocreatef(_net_n, _net_m, _net_x, _net_diffstep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetalgonbi(*functionargs):
    # unpack inputs
    state, frontsize, polishsolutions = functionargs
    friendly_form = False

    # convert to .NET types
    _net_state = state.ptr
    _net_frontsize = frontsize
    _net_polishsolutions = polishsolutions
    try:

        # call function
        _net_alglib.minmosetalgonbi(_net_state, _net_frontsize, _net_polishsolutions)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetbc(*functionargs):
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_bndl = net_from_list(bndl, DT_REAL, "ALGLIB: parameter 'bndl' of 'xalglib.minmosetbc' must be real vector")
    _net_bndu = net_from_list(bndu, DT_REAL, "ALGLIB: parameter 'bndu' of 'xalglib.minmosetbc' must be real vector")
    try:

        # call function
        _net_alglib.minmosetbc(_net_state, _net_bndl, _net_bndu)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetlc2dense(*functionargs):
    # unpack inputs
    if len(functionargs)==5:
        # full-form call
        state, a, al, au, k = functionargs
        ...
        # short-form call
        state, a, al, au = functionargs
        k = check_equality_and_get([safe_rows("'minmosetlc2dense': incorrect parameters",a), safe_len("'minmosetlc2dense': incorrect parameters",al), safe_len("'minmosetlc2dense': incorrect parameters",au)],"Error while calling 'minmosetlc2dense': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'minmosetlc2dense': function must have 5 or 4 parameters")

    # convert to .NET types
    _net_state = state.ptr
    _net_a = net_from_listlist(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.minmosetlc2dense' must be real matrix")
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.minmosetlc2dense' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.minmosetlc2dense' must be real vector")
    _net_k = k
    try:

        # call function
        _net_alglib.minmosetlc2dense(_net_state, _net_a, _net_al, _net_au, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetlc2(*functionargs):
    ...
    _net_state = state.ptr
    _net_a = a.ptr
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.minmosetlc2' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.minmosetlc2' must be real vector")
    _net_k = k
    try:

        # call function
        _net_alglib.minmosetlc2(_net_state, _net_a, _net_al, _net_au, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetlc2mixed(*functionargs):
    # unpack inputs
    state, sparsea, ksparse, densea, kdense, al, au = functionargs
    friendly_form = False
    ...
    _net_sparsea = sparsea.ptr
    _net_ksparse = ksparse
    _net_densea = net_from_listlist(densea, DT_REAL, "ALGLIB: parameter 'densea' of 'xalglib.minmosetlc2mixed' must be real matrix")
    _net_kdense = kdense
    _net_al = net_from_list(al, DT_REAL, "ALGLIB: parameter 'al' of 'xalglib.minmosetlc2mixed' must be real vector")
    _net_au = net_from_list(au, DT_REAL, "ALGLIB: parameter 'au' of 'xalglib.minmosetlc2mixed' must be real vector")
    try:

        # call function
        _net_alglib.minmosetlc2mixed(_net_state, _net_sparsea, _net_ksparse, _net_densea, _net_kdense, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmoaddlc2dense(*functionargs):
    # unpack inputs
    state, a, al, au = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.minmoaddlc2dense' must be real vector")
    _net_al = al
    _net_au = au
    try:

        # call function
        _net_alglib.minmoaddlc2dense(_net_state, _net_a, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmoaddlc2(*functionargs):
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_idxa = net_from_list(idxa, DT_INT, "ALGLIB: parameter 'idxa' of 'xalglib.minmoaddlc2' must be int vector")
    _net_vala = net_from_list(vala, DT_REAL, "ALGLIB: parameter 'vala' of 'xalglib.minmoaddlc2' must be real vector")
    _net_nnz = nnz
    _net_al = al
    _net_au = au
    try:
        ...
        # call function
        _net_alglib.minmoaddlc2(_net_state, _net_idxa, _net_vala, _net_nnz, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmoaddlc2sparsefromdense(*functionargs):
    # unpack inputs
    state, da, al, au = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_da = net_from_list(da, DT_REAL, "ALGLIB: parameter 'da' of 'xalglib.minmoaddlc2sparsefromdense' must be real vector")
    _net_al = al
    _net_au = au
    try:

        # call function
        _net_alglib.minmoaddlc2sparsefromdense(_net_state, _net_da, _net_al, _net_au)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetnlc2(*functionargs):
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_nl = net_from_list(nl, DT_REAL, "ALGLIB: parameter 'nl' of 'xalglib.minmosetnlc2' must be real vector")
    _net_nu = net_from_list(nu, DT_REAL, "ALGLIB: parameter 'nu' of 'xalglib.minmosetnlc2' must be real vector")
    _net_nnlc = nnlc
    try:

        # call function
        _net_alglib.minmosetnlc2(_net_state, _net_nl, _net_nu, _net_nnlc)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetcond(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.minmosetcond(_net_state, _net_epsx, _net_maxits)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetscale(*functionargs):
    # unpack inputs
    state, s = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_s = net_from_list(s, DT_REAL, "ALGLIB: parameter 's' of 'xalglib.minmosetscale' must be real vector")
    try:

        # call function
        _net_alglib.minmosetscale(_net_state, _net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmosetxrep(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.minmosetxrep(_net_state, _net_needxrep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmooptimize_v(state, fvec, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minmooptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return


def minmooptimize_j(state, jac, rep = None, param = None):
    _pxy_jac  = _ipynet_proxy_jac(jac,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minmooptimize(state.ptr, _pxy_jac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return


def minmoresults(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_paretofront, _net_frontsize, _net_rep = _net_alglib.minmoresults(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    paretofront = listlist_from_net(_net_paretofront, DT_REAL)
    frontsize = _net_frontsize
    rep = minmoreport_from_net(_net_rep)

    # return
    return (paretofront, frontsize, rep)

def minmorequesttermination(*functionargs):
    # unpack inputs
    state,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.minmorequesttermination(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def minmorestartfrom(*functionargs):
    # unpack inputs
    state, x = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_state = state.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.minmorestartfrom' must be real vector")
    try:

        # call function
        _net_alglib.minmorestartfrom(_net_state, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_minnsreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.cerr = x.cerr
    r.lcerr = x.lcerr
...
def minnsoptimize_v(state, fvec, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minnsoptimize(state.ptr, _pxy_fvec.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minnsoptimize_j(state, jac, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minnsoptimize(state.ptr, _pxy_jac.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minnsresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minnsresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minnsreport_inplace(_net_rep, rep)
...
def net_to_minasareport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
    r.activeconstraints = x.activeconstraints
...
def minasaoptimize_g(state, grad, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minasaoptimize(state.ptr, _pxy_grad.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minasaresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minasaresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minasareport_inplace(_net_rep, rep)
...
def net_to_minbcreport_inplace(x,r):
    r.iterationscount = x.iterationscount
    r.nfev = x.nfev
    r.varidx = x.varidx
    r.terminationtype = x.terminationtype
...
def minbcoptimize_f(state, func, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minbcoptimize(state.ptr, _pxy_func.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minbcoptimize_g(state, grad, rep = None, param = None):
    ...
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.minbcoptimize(state.ptr, _pxy_grad.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def minbcresultsbuf(*functionargs):
    ...
    try:
        ...
        # call function
        _net_x = _net_alglib.minbcresultsbuf(_net_state, _net_x, _net_rep)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    x = list_from_net(_net_x, DT_REAL)
    net_to_minbcreport_inplace(_net_rep, rep)
...
def net_to_odesolverreport_inplace(x,r):
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
    return
...
def rmatrixinvupdatesimple(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixinvupdatesimple(_net_inva, _net_n, _net_updrow, _net_updcolumn, _net_updval)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_inva, DT_REAL, inva)
...
def rmatrixinvupdaterow(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixinvupdaterow(_net_inva, _net_n, _net_updrow, _net_v)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixinvupdatecolumn(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixinvupdatecolumn(_net_inva, _net_n, _net_updcolumn, _net_u)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixinvupdateuv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rmatrixinvupdateuv(_net_inva, _net_n, _net_u, _net_v)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rmatrixschur(*functionargs):
    ...
    try:
        ...
        # call function
        _net_result, _net_s = _net_alglib.rmatrixschur(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def smatrixgevdreduce(*functionargs):
    ...
    try:
        ...
        # call function
        _net_result, _net_r, _net_isupperr = _net_alglib.smatrixgevdreduce(_net_a, _net_n, _net_isuppera, _net_b, _net_isupperb, _net_problemtype)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spdmatrixdet(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        raise RuntimeError("Error while calling 'spdmatrixdet': function must have 3 or 2 parameters")
...
def net_to_autogkreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.nfev = x.nfev
    r.nintervals = x.nintervals
    return
...
def rankdata(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rankdata(_net_xy, _net_npoints, _net_nfeatures)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    net_to_listlist_inplace(_net_xy, DT_REAL, xy)
...
def rankdatacentered(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rankdatacentered(_net_xy, _net_npoints, _net_nfeatures)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_idwreport_inplace(x,r):
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
    r.maxerror = x.maxerror
    r.r2 = x.r2
...
def idwpeekprogress(*functionargs):
    # unpack inputs
    s,  = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_result = _net_alglib.idwpeekprogress(_net_s)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def idwgridcalc2v(*functionargs):
    # unpack inputs
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x0 = net_from_list(x0, DT_REAL, "ALGLIB: parameter 'x0' of 'xalglib.idwgridcalc2v' must be real vector")
    _net_n0 = n0
    _net_x1 = net_from_list(x1, DT_REAL, "ALGLIB: parameter 'x1' of 'xalglib.idwgridcalc2v' must be real vector")
    _net_n1 = n1
    try:

        # call function
        _net_y = _net_alglib.idwgridcalc2v(_net_s, _net_x0, _net_n0, _net_x1, _net_n1)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def idwgridcalc2vsubset(*functionargs):
    # unpack inputs
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x0 = net_from_list(x0, DT_REAL, "ALGLIB: parameter 'x0' of 'xalglib.idwgridcalc2vsubset' must be real vector")
    _net_n0 = n0
    _net_x1 = net_from_list(x1, DT_REAL, "ALGLIB: parameter 'x1' of 'xalglib.idwgridcalc2vsubset' must be real vector")
    _net_n1 = n1
    _net_flagy = net_from_list(flagy, DT_BOOL, "ALGLIB: parameter 'flagy' of 'xalglib.idwgridcalc2vsubset' must be bool vector")
    try:

        # call function
        _net_y = _net_alglib.idwgridcalc2vsubset(_net_s, _net_x0, _net_n0, _net_x1, _net_n1, _net_flagy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_spline1dfitreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.taskrcond = x.taskrcond
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
...
def spline1dserialize(obj):
    try:
        return _net_alglib.spline1dserialize(obj.ptr)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

def spline1dunserialize(s_in):
    try:
        return spline1dinterpolant(_net_alglib.spline1dunserialize(s_in))
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dbuildlinearbuf(*functionargs):
    # unpack inputs
    if len(functionargs)==4:
        # full-form call
        x, y, n, c = functionargs
        friendly_form = False
    elif len(functionargs)==3:
        # short-form call
        x, y, c = functionargs
        n = check_equality_and_get([safe_len("'spline1dbuildlinearbuf': incorrect parameters",x), safe_len("'spline1dbuildlinearbuf': incorrect parameters",y)],"Error while calling 'spline1dbuildlinearbuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline1dbuildlinearbuf': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline1dbuildlinearbuf' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline1dbuildlinearbuf' must be real vector")
    _net_n = n
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline1dbuildlinearbuf(_net_x, _net_y, _net_n, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dbuildhermitebuf(*functionargs):
    # unpack inputs
    if len(functionargs)==5:
        # full-form call
        x, y, d, n, c = functionargs
        friendly_form = False
    elif len(functionargs)==4:
        # short-form call
        x, y, d, c = functionargs
        n = check_equality_and_get([safe_len("'spline1dbuildhermitebuf': incorrect parameters",x), safe_len("'spline1dbuildhermitebuf': incorrect parameters",y), safe_len("'spline1dbuildhermitebuf': incorrect parameters",d)],"Error while calling 'spline1dbuildhermitebuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline1dbuildhermitebuf': function must have 5 or 4 parameters")

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline1dbuildhermitebuf' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline1dbuildhermitebuf' must be real vector")
    _net_d = net_from_list(d, DT_REAL, "ALGLIB: parameter 'd' of 'xalglib.spline1dbuildhermitebuf' must be real vector")
    _net_n = n
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline1dbuildhermitebuf(_net_x, _net_y, _net_d, _net_n, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dbuildakimamod(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        x, y = functionargs
        n = check_equality_and_get([safe_len("'spline1dbuildakimamod': incorrect parameters",x), safe_len("'spline1dbuildakimamod': incorrect parameters",y)],"Error while calling 'spline1dbuildakimamod': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline1dbuildakimamod': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline1dbuildakimamod' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline1dbuildakimamod' must be real vector")
    _net_n = n
    try:

        # call function
        _net_c = _net_alglib.spline1dbuildakimamod(_net_x, _net_y, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_polynomialfitreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.taskrcond = x.taskrcond
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
...
def net_to_barycentricfitreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.taskrcond = x.taskrcond
    r.dbest = x.dbest
    r.rmserror = x.rmserror
...
def net_to_lsfitreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.taskrcond = x.taskrcond
    r.iterationscount = x.iterationscount
    r.varidx = x.varidx
...
def polynomialfit(*functionargs):
    ...
    try:
        ...
        # call function
        _net_p, _net_rep = _net_alglib.polynomialfit(_net_x, _net_y, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def polynomialfitwc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_p, _net_rep = _net_alglib.polynomialfitwc(_net_x, _net_y, _net_w, _net_n, _net_xc, _net_yc, _net_dc, _net_k, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def barycentricfitfloaterhormannwc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_b, _net_rep = _net_alglib.barycentricfitfloaterhormannwc(_net_x, _net_y, _net_w, _net_n, _net_xc, _net_yc, _net_dc, _net_k, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def barycentricfitfloaterhormann(*functionargs):
    ...
    try:
        ...
        # call function
        _net_b, _net_rep = _net_alglib.barycentricfitfloaterhormann(_net_x, _net_y, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dfitcubicwc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_s, _net_rep = _net_alglib.spline1dfitcubicwc(_net_x, _net_y, _net_w, _net_n, _net_xc, _net_yc, _net_dc, _net_k, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dfithermitewc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_s, _net_rep = _net_alglib.spline1dfithermitewc(_net_x, _net_y, _net_w, _net_n, _net_xc, _net_yc, _net_dc, _net_k, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dfithermitedeprecated(*functionargs):
    # unpack inputs
    if len(functionargs)==4:
        # full-form call
        x, y, n, m = functionargs
        ...
        # short-form call
        x, y, m = functionargs
        n = check_equality_and_get([safe_len("'spline1dfithermitedeprecated': incorrect parameters",x), safe_len("'spline1dfithermitedeprecated': incorrect parameters",y)],"Error while calling 'spline1dfithermitedeprecated': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline1dfithermitedeprecated': function must have 4 or 3 parameters")

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline1dfithermitedeprecated' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline1dfithermitedeprecated' must be real vector")
    _net_n = n
    _net_m = m
    try:

        # call function
        _net_s, _net_rep = _net_alglib.spline1dfithermitedeprecated(_net_x, _net_y, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitlinearw(*functionargs):
    ...
    try:
        ...
        # call function
        _net_c, _net_rep = _net_alglib.lsfitlinearw(_net_y, _net_w, _net_fmatrix, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitlinearwc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_c, _net_rep = _net_alglib.lsfitlinearwc(_net_y, _net_w, _net_fmatrix, _net_cmatrix, _net_n, _net_m, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitlinear(*functionargs):
    ...
    try:
        ...
        # call function
        _net_c, _net_rep = _net_alglib.lsfitlinear(_net_y, _net_fmatrix, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitlinearc(*functionargs):
    ...
    try:
        ...
        # call function
        _net_c, _net_rep = _net_alglib.lsfitlinearc(_net_y, _net_fmatrix, _net_cmatrix, _net_n, _net_m, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitcreatewfg(*functionargs):
    ...
    if len(functionargs)==7:
        ...
        raise RuntimeError("Error while calling 'lsfitcreatewfg': function must have 7 or 4 parameters")
    ...
    try:
        ...
        # call function
        _net_state = _net_alglib.lsfitcreatewfg(_net_x, _net_y, _net_w, _net_c, _net_n, _net_m, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitcreatefg(*functionargs):
    ...
    if len(functionargs)==6:
        ...
        raise RuntimeError("Error while calling 'lsfitcreatefg': function must have 6 or 3 parameters")
    ...
    try:
        ...
        # call function
        _net_state = _net_alglib.lsfitcreatefg(_net_x, _net_y, _net_c, _net_n, _net_m, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitsetnonmonotonicsteps(*functionargs):
    # unpack inputs
    state, cnt = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.lsfitsetnonmonotonicsteps(_net_state, _net_cnt)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitsetnumdiff(*functionargs):
    # unpack inputs
    state, formulatype = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.lsfitsetnumdiff(_net_state, _net_formulatype)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lsfitfit_f(state, func, rep = None, param = None):
    _pxy_func = _ipynet_proxy_pfunc(func,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.lsfitfit(state.ptr, _pxy_func.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def lsfitfit_fg(state, func, grad, rep = None, param = None):
    _pxy_func = _ipynet_proxy_pfunc(func,param)
    _pxy_grad = _ipynet_proxy_pgrad(grad,param)
    _pxy_rep  = _ipynet_proxy_rep(rep,param)
    try:
        _net_alglib.lsfitfit(state.ptr, _pxy_func.get_delegate(), _pxy_grad.get_delegate(), _pxy_rep.get_delegate(), None)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    return
...
def lsfitresults(*functionargs):
    ...
    try:
        ...
        # call function
        _net_c, _net_rep = _net_alglib.lsfitresults(_net_state)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fitspherex(*functionargs):
    # unpack inputs
    xy, npoints, nx, problemtype, epsx, aulits = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_cx, _net_rlo, _net_rhi = _net_alglib.fitspherex(_net_xy, _net_npoints, _net_nx, _net_problemtype, _net_epsx, _net_aulits)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_spline2dfitreport_inplace(x,r):
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
    r.maxerror = x.maxerror
    r.r2 = x.r2
...
def spline2ddiff(*functionargs):
    ...
    try:
        ...
        # call function
        _net_f, _net_fx, _net_fy = _net_alglib.spline2ddiff(_net_c, _net_x, _net_y)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2ddiff2(*functionargs):
    # unpack inputs
    c, x, y = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_f, _net_fx, _net_fy, _net_fxx, _net_fxy, _net_fyy = _net_alglib.spline2ddiff2(_net_c, _net_x, _net_y)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2ddiffvi(*functionargs):
    ...
    try:
        ...
        # call function
        _net_f, _net_fx, _net_fy = _net_alglib.spline2ddiffvi(_net_c, _net_x, _net_y, _net_i)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2ddiff2vi(*functionargs):
    # unpack inputs
    c, x, y, i = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_f, _net_fx, _net_fy, _net_fxx, _net_fxy, _net_fyy = _net_alglib.spline2ddiff2vi(_net_c, _net_x, _net_y, _net_i)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbilinearvbuf(*functionargs):
    # unpack inputs
    x, n, y, m, f, d, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildbilinearvbuf' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildbilinearvbuf' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildbilinearvbuf' must be real vector")
    _net_d = d
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline2dbuildbilinearvbuf(_net_x, _net_n, _net_y, _net_m, _net_f, _net_d, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbilinearmissing(*functionargs):
    # unpack inputs
    x, n, y, m, f, missing, d = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildbilinearmissing' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildbilinearmissing' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildbilinearmissing' must be real vector")
    _net_missing = net_from_list(missing, DT_BOOL, "ALGLIB: parameter 'missing' of 'xalglib.spline2dbuildbilinearmissing' must be bool vector")
    _net_d = d
    try:

        # call function
        _net_c = _net_alglib.spline2dbuildbilinearmissing(_net_x, _net_n, _net_y, _net_m, _net_f, _net_missing, _net_d)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbilinearmissingbuf(*functionargs):
    # unpack inputs
    x, n, y, m, f, missing, d, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildbilinearmissingbuf' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildbilinearmissingbuf' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildbilinearmissingbuf' must be real vector")
    _net_missing = net_from_list(missing, DT_BOOL, "ALGLIB: parameter 'missing' of 'xalglib.spline2dbuildbilinearmissingbuf' must be bool vector")
    _net_d = d
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline2dbuildbilinearmissingbuf(_net_x, _net_n, _net_y, _net_m, _net_f, _net_missing, _net_d, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbicubicv(*functionargs):
    ...
    if len(functionargs)==6:
        ...
        n = check_equality_and_get([safe_len("'spline2dbuildbicubicv': incorrect parameters",x)],"Error while calling 'spline2dbuildbicubicv': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_len("'spline2dbuildbicubicv': incorrect parameters",y)],"Error while calling 'spline2dbuildbicubicv': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline2dbuildbicubicv': function must have 6 or 4 parameters")
...
def spline2dbuildclampedv(*functionargs):
    ...
    if len(functionargs)==15:
        ...
        n = check_equality_and_get([safe_len("'spline2dbuildclampedv': incorrect parameters",x)],"Error while calling 'spline2dbuildclampedv': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_len("'spline2dbuildclampedv': incorrect parameters",y)],"Error while calling 'spline2dbuildclampedv': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline2dbuildclampedv': function must have 15 or 13 parameters")

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_m = m
    _net_bndbtm = net_from_list(bndbtm, DT_REAL, "ALGLIB: parameter 'bndbtm' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_bndtypebtm = bndtypebtm
    _net_bndtop = net_from_list(bndtop, DT_REAL, "ALGLIB: parameter 'bndtop' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_bndtypetop = bndtypetop
    _net_bndlft = net_from_list(bndlft, DT_REAL, "ALGLIB: parameter 'bndlft' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_bndtypelft = bndtypelft
    _net_bndrgt = net_from_list(bndrgt, DT_REAL, "ALGLIB: parameter 'bndrgt' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_bndtypergt = bndtypergt
    _net_mixedd = net_from_list(mixedd, DT_REAL, "ALGLIB: parameter 'mixedd' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildclampedv' must be real vector")
    _net_d = d
    try:

        # call function
        _net_c = _net_alglib.spline2dbuildclampedv(_net_x, _net_n, _net_y, _net_m, _net_bndbtm, _net_bndtypebtm, _net_bndtop, _net_bndtypetop, _net_bndlft, _net_bndtypelft, _net_bndrgt, _net_bndtypergt, _net_mixedd, _net_f, _net_d)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildhermitev(*functionargs):
    ...
    if len(functionargs)==9:
        ...
        n = check_equality_and_get([safe_len("'spline2dbuildhermitev': incorrect parameters",x)],"Error while calling 'spline2dbuildhermitev': looks like one of arguments has wrong size")
        m = check_equality_and_get([safe_len("'spline2dbuildhermitev': incorrect parameters",y)],"Error while calling 'spline2dbuildhermitev': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'spline2dbuildhermitev': function must have 9 or 7 parameters")

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildhermitev' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildhermitev' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildhermitev' must be real vector")
    _net_dfdx = net_from_list(dfdx, DT_REAL, "ALGLIB: parameter 'dfdx' of 'xalglib.spline2dbuildhermitev' must be real vector")
    _net_dfdy = net_from_list(dfdy, DT_REAL, "ALGLIB: parameter 'dfdy' of 'xalglib.spline2dbuildhermitev' must be real vector")
    _net_d2fdxdy = net_from_list(d2fdxdy, DT_REAL, "ALGLIB: parameter 'd2fdxdy' of 'xalglib.spline2dbuildhermitev' must be real vector")
    _net_d = d
    try:

        # call function
        _net_c = _net_alglib.spline2dbuildhermitev(_net_x, _net_n, _net_y, _net_m, _net_f, _net_dfdx, _net_dfdy, _net_d2fdxdy, _net_d)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbicubicvbuf(*functionargs):
    # unpack inputs
    x, n, y, m, f, d, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildbicubicvbuf' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildbicubicvbuf' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildbicubicvbuf' must be real vector")
    _net_d = d
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline2dbuildbicubicvbuf(_net_x, _net_n, _net_y, _net_m, _net_f, _net_d, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbicubicmissing(*functionargs):
    # unpack inputs
    x, n, y, m, f, missing, d = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildbicubicmissing' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildbicubicmissing' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildbicubicmissing' must be real vector")
    _net_missing = net_from_list(missing, DT_BOOL, "ALGLIB: parameter 'missing' of 'xalglib.spline2dbuildbicubicmissing' must be bool vector")
    _net_d = d
    try:

        # call function
        _net_c = _net_alglib.spline2dbuildbicubicmissing(_net_x, _net_n, _net_y, _net_m, _net_f, _net_missing, _net_d)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline2dbuildbicubicmissingbuf(*functionargs):
    # unpack inputs
    x, n, y, m, f, missing, d, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline2dbuildbicubicmissingbuf' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline2dbuildbicubicmissingbuf' must be real vector")
    _net_m = m
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline2dbuildbicubicmissingbuf' must be real vector")
    _net_missing = net_from_list(missing, DT_BOOL, "ALGLIB: parameter 'missing' of 'xalglib.spline2dbuildbicubicmissingbuf' must be bool vector")
    _net_d = d
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline2dbuildbicubicmissingbuf(_net_x, _net_n, _net_y, _net_m, _net_f, _net_missing, _net_d, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline3dbuildtrilinearvbuf(*functionargs):
    # unpack inputs
    x, n, y, m, z, l, f, d, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.spline3dbuildtrilinearvbuf' must be real vector")
    _net_n = n
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.spline3dbuildtrilinearvbuf' must be real vector")
    _net_m = m
    _net_z = net_from_list(z, DT_REAL, "ALGLIB: parameter 'z' of 'xalglib.spline3dbuildtrilinearvbuf' must be real vector")
    _net_l = l
    _net_f = net_from_list(f, DT_REAL, "ALGLIB: parameter 'f' of 'xalglib.spline3dbuildtrilinearvbuf' must be real vector")
    _net_d = d
    _net_c = c.ptr
    try:

        # call function
        _net_alglib.spline3dbuildtrilinearvbuf(_net_x, _net_n, _net_y, _net_m, _net_z, _net_l, _net_f, _net_d, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dfitcubic(*functionargs):
    ...
    try:
        ...
        # call function
        _net_s, _net_rep = _net_alglib.spline1dfitcubic(_net_x, _net_y, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def spline1dfithermite(*functionargs):
    ...
    try:
        ...
        # call function
        _net_s, _net_rep = _net_alglib.spline1dfithermite(_net_x, _net_y, _net_n, _net_m)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_rbfreport_inplace(x,r):
    r.rmserror = x.rmserror
    r.maxerror = x.maxerror
    r.arows = x.arows
    r.acols = x.acols
...
def rbfsetalgothinplatespline(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        s,  = functionargs
        lambdav = check_equality_and_get([0.0],"Error while calling 'rbfsetalgothinplatespline': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rbfsetalgothinplatespline': function must have 2 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.rbfsetalgothinplatespline(_net_s, _net_lambdav)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfsetalgomultiquadricmanual(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        lambdav = check_equality_and_get([0.0],"Error while calling 'rbfsetalgomultiquadricmanual': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rbfsetalgomultiquadricmanual': function must have 3 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.rbfsetalgomultiquadricmanual(_net_s, _net_alpha, _net_lambdav)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfsetalgomultiquadricauto(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        s,  = functionargs
        lambdav = check_equality_and_get([0.0],"Error while calling 'rbfsetalgomultiquadricauto': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rbfsetalgomultiquadricauto': function must have 2 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.rbfsetalgomultiquadricauto(_net_s, _net_lambdav)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfsetalgobiharmonic(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        s,  = functionargs
        lambdav = check_equality_and_get([0.0],"Error while calling 'rbfsetalgobiharmonic': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'rbfsetalgobiharmonic': function must have 2 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_alglib.rbfsetalgobiharmonic(_net_s, _net_lambdav)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfsetv3tol(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.rbfsetv3tol(_net_s, _net_tol)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfdiff1(*functionargs):
    ...
    try:
        ...
        # call function
        _net_y, _net_dy0 = _net_alglib.rbfdiff1(_net_s, _net_x0)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfdiff2(*functionargs):
    ...
    try:
        ...
        # call function
        _net_y, _net_dy0, _net_dy1 = _net_alglib.rbfdiff2(_net_s, _net_x0, _net_x1)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfdiff3(*functionargs):
    ...
    try:
        ...
        # call function
        _net_y, _net_dy0, _net_dy1, _net_dy2 = _net_alglib.rbfdiff3(_net_s, _net_x0, _net_x1, _net_x2)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    # return
    return (y, dy0, dy1, dy2)

def rbfsetfastevaltol(*functionargs):
    # unpack inputs
    s, tol = functionargs
    friendly_form = False
    ...
    try:
        ...
        # call function
        _net_alglib.rbfsetfastevaltol(_net_s, _net_tol)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbffastcalc(*functionargs):
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbffastcalc' must be real vector")
    try:

        # call function
        _net_y = _net_alglib.rbffastcalc(_net_s, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfdiff(*functionargs):
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbfdiff' must be real vector")
    try:

        # call function
        _net_y, _net_dy = _net_alglib.rbfdiff(_net_s, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    y = list_from_net(_net_y, DT_REAL)
    dy = list_from_net(_net_dy, DT_REAL)

    # return
    return (y, dy)

def rbfhess(*functionargs):
    # unpack inputs
    s, x = functionargs
    friendly_form = False
    ...
    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbfhess' must be real vector")
    try:

        # call function
        _net_y, _net_dy, _net_d2y = _net_alglib.rbfhess(_net_s, _net_x)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
    ...
    y = list_from_net(_net_y, DT_REAL)
    dy = list_from_net(_net_dy, DT_REAL)
    d2y = list_from_net(_net_d2y, DT_REAL)
...
def rbfdiffbuf(*functionargs):
    # unpack inputs
    s, x, y, dy = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbfdiffbuf' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.rbfdiffbuf' must be real vector")
    _net_dy = net_from_list(dy, DT_REAL, "ALGLIB: parameter 'dy' of 'xalglib.rbfdiffbuf' must be real vector")
    try:

        # call function
        _net_y, _net_dy = _net_alglib.rbfdiffbuf(_net_s, _net_x, _net_y, _net_dy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbfhessbuf(*functionargs):
    # unpack inputs
    s, x, y, dy, d2y = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = s.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbfhessbuf' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.rbfhessbuf' must be real vector")
    _net_dy = net_from_list(dy, DT_REAL, "ALGLIB: parameter 'dy' of 'xalglib.rbfhessbuf' must be real vector")
    _net_d2y = net_from_list(d2y, DT_REAL, "ALGLIB: parameter 'd2y' of 'xalglib.rbfhessbuf' must be real vector")
    try:

        # call function
        _net_y, _net_dy, _net_d2y = _net_alglib.rbfhessbuf(_net_s, _net_x, _net_y, _net_dy, _net_d2y)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbftsdiffbuf(*functionargs):
    # unpack inputs
    s, buf, x, y, dy = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = s.ptr
    _net_buf = buf.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbftsdiffbuf' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.rbftsdiffbuf' must be real vector")
    _net_dy = net_from_list(dy, DT_REAL, "ALGLIB: parameter 'dy' of 'xalglib.rbftsdiffbuf' must be real vector")
    try:

        # call function
        _net_y, _net_dy = _net_alglib.rbftsdiffbuf(_net_s, _net_buf, _net_x, _net_y, _net_dy)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def rbftshessbuf(*functionargs):
    # unpack inputs
    s, buf, x, y, dy, d2y = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = s.ptr
    _net_buf = buf.ptr
    _net_x = net_from_list(x, DT_REAL, "ALGLIB: parameter 'x' of 'xalglib.rbftshessbuf' must be real vector")
    _net_y = net_from_list(y, DT_REAL, "ALGLIB: parameter 'y' of 'xalglib.rbftshessbuf' must be real vector")
    _net_dy = net_from_list(dy, DT_REAL, "ALGLIB: parameter 'dy' of 'xalglib.rbftshessbuf' must be real vector")
    _net_d2y = net_from_list(d2y, DT_REAL, "ALGLIB: parameter 'd2y' of 'xalglib.rbftshessbuf' must be real vector")
    try:

        # call function
        _net_y, _net_dy, _net_d2y = _net_alglib.rbftshessbuf(_net_s, _net_buf, _net_x, _net_y, _net_dy, _net_d2y)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fftc1d(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.fftc1d(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fftc1dinv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.fftc1dinv(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fftr1dbuf(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        a, f = functionargs
        n = check_equality_and_get([safe_len("'fftr1dbuf': incorrect parameters",a)],"Error while calling 'fftr1dbuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'fftr1dbuf': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.fftr1dbuf' must be real vector")
    _net_n = n
    _net_f = net_from_list(f, DT_COMPLEX, "ALGLIB: parameter 'f' of 'xalglib.fftr1dbuf' must be complex vector")
    try:

        # call function
        _net_f = _net_alglib.fftr1dbuf(_net_a, _net_n, _net_f)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fftr1dinvbuf(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        f, a = functionargs
        n = check_equality_and_get([safe_len("'fftr1dinvbuf': incorrect parameters",f)],"Error while calling 'fftr1dinvbuf': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'fftr1dinvbuf': function must have 3 or 2 parameters")

    # convert to .NET types
    _net_f = net_from_list(f, DT_COMPLEX, "ALGLIB: parameter 'f' of 'xalglib.fftr1dinvbuf' must be complex vector")
    _net_n = n
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.fftr1dinvbuf' must be real vector")
    try:

        # call function
        _net_a = _net_alglib.fftr1dinvbuf(_net_f, _net_n, _net_a)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fhtr1d(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.fhtr1d(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fhtr1dinv(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.fhtr1dinv(_net_a, _net_n)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convc1dbuf(*functionargs):
    # unpack inputs
    a, m, b, n, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_COMPLEX, "ALGLIB: parameter 'a' of 'xalglib.convc1dbuf' must be complex vector")
    _net_m = m
    _net_b = net_from_list(b, DT_COMPLEX, "ALGLIB: parameter 'b' of 'xalglib.convc1dbuf' must be complex vector")
    _net_n = n
    _net_r = net_from_list(r, DT_COMPLEX, "ALGLIB: parameter 'r' of 'xalglib.convc1dbuf' must be complex vector")
    try:

        # call function
        _net_r = _net_alglib.convc1dbuf(_net_a, _net_m, _net_b, _net_n, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convc1dinvbuf(*functionargs):
    # unpack inputs
    a, m, b, n, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_COMPLEX, "ALGLIB: parameter 'a' of 'xalglib.convc1dinvbuf' must be complex vector")
    _net_m = m
    _net_b = net_from_list(b, DT_COMPLEX, "ALGLIB: parameter 'b' of 'xalglib.convc1dinvbuf' must be complex vector")
    _net_n = n
    _net_r = net_from_list(r, DT_COMPLEX, "ALGLIB: parameter 'r' of 'xalglib.convc1dinvbuf' must be complex vector")
    try:

        # call function
        _net_r = _net_alglib.convc1dinvbuf(_net_a, _net_m, _net_b, _net_n, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convc1dcircularbuf(*functionargs):
    # unpack inputs
    s, m, r, n, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = net_from_list(s, DT_COMPLEX, "ALGLIB: parameter 's' of 'xalglib.convc1dcircularbuf' must be complex vector")
    _net_m = m
    _net_r = net_from_list(r, DT_COMPLEX, "ALGLIB: parameter 'r' of 'xalglib.convc1dcircularbuf' must be complex vector")
    _net_n = n
    _net_c = net_from_list(c, DT_COMPLEX, "ALGLIB: parameter 'c' of 'xalglib.convc1dcircularbuf' must be complex vector")
    try:

        # call function
        _net_c = _net_alglib.convc1dcircularbuf(_net_s, _net_m, _net_r, _net_n, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convc1dcircularinvbuf(*functionargs):
    # unpack inputs
    a, m, b, n, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_COMPLEX, "ALGLIB: parameter 'a' of 'xalglib.convc1dcircularinvbuf' must be complex vector")
    _net_m = m
    _net_b = net_from_list(b, DT_COMPLEX, "ALGLIB: parameter 'b' of 'xalglib.convc1dcircularinvbuf' must be complex vector")
    _net_n = n
    _net_r = net_from_list(r, DT_COMPLEX, "ALGLIB: parameter 'r' of 'xalglib.convc1dcircularinvbuf' must be complex vector")
    try:

        # call function
        _net_r = _net_alglib.convc1dcircularinvbuf(_net_a, _net_m, _net_b, _net_n, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convr1dbuf(*functionargs):
    # unpack inputs
    a, m, b, n, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.convr1dbuf' must be real vector")
    _net_m = m
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.convr1dbuf' must be real vector")
    _net_n = n
    _net_r = net_from_list(r, DT_REAL, "ALGLIB: parameter 'r' of 'xalglib.convr1dbuf' must be real vector")
    try:

        # call function
        _net_r = _net_alglib.convr1dbuf(_net_a, _net_m, _net_b, _net_n, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convr1dinvbuf(*functionargs):
    # unpack inputs
    a, m, b, n, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.convr1dinvbuf' must be real vector")
    _net_m = m
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.convr1dinvbuf' must be real vector")
    _net_n = n
    _net_r = net_from_list(r, DT_REAL, "ALGLIB: parameter 'r' of 'xalglib.convr1dinvbuf' must be real vector")
    try:

        # call function
        _net_r = _net_alglib.convr1dinvbuf(_net_a, _net_m, _net_b, _net_n, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convr1dcircularbuf(*functionargs):
    # unpack inputs
    s, m, r, n, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_s = net_from_list(s, DT_REAL, "ALGLIB: parameter 's' of 'xalglib.convr1dcircularbuf' must be real vector")
    _net_m = m
    _net_r = net_from_list(r, DT_REAL, "ALGLIB: parameter 'r' of 'xalglib.convr1dcircularbuf' must be real vector")
    _net_n = n
    _net_c = net_from_list(c, DT_REAL, "ALGLIB: parameter 'c' of 'xalglib.convr1dcircularbuf' must be real vector")
    try:

        # call function
        _net_c = _net_alglib.convr1dcircularbuf(_net_s, _net_m, _net_r, _net_n, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def convr1dcircularinvbuf(*functionargs):
    # unpack inputs
    a, m, b, n, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_a = net_from_list(a, DT_REAL, "ALGLIB: parameter 'a' of 'xalglib.convr1dcircularinvbuf' must be real vector")
    _net_m = m
    _net_b = net_from_list(b, DT_REAL, "ALGLIB: parameter 'b' of 'xalglib.convr1dcircularinvbuf' must be real vector")
    _net_n = n
    _net_r = net_from_list(r, DT_REAL, "ALGLIB: parameter 'r' of 'xalglib.convr1dcircularinvbuf' must be real vector")
    try:

        # call function
        _net_r = _net_alglib.convr1dcircularinvbuf(_net_a, _net_m, _net_b, _net_n, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def corrc1dbuf(*functionargs):
    # unpack inputs
    signal, n, pattern, m, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_signal = net_from_list(signal, DT_COMPLEX, "ALGLIB: parameter 'signal' of 'xalglib.corrc1dbuf' must be complex vector")
    _net_n = n
    _net_pattern = net_from_list(pattern, DT_COMPLEX, "ALGLIB: parameter 'pattern' of 'xalglib.corrc1dbuf' must be complex vector")
    _net_m = m
    _net_r = net_from_list(r, DT_COMPLEX, "ALGLIB: parameter 'r' of 'xalglib.corrc1dbuf' must be complex vector")
    try:

        # call function
        _net_r = _net_alglib.corrc1dbuf(_net_signal, _net_n, _net_pattern, _net_m, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def corrc1dcircularbuf(*functionargs):
    # unpack inputs
    signal, m, pattern, n, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_signal = net_from_list(signal, DT_COMPLEX, "ALGLIB: parameter 'signal' of 'xalglib.corrc1dcircularbuf' must be complex vector")
    _net_m = m
    _net_pattern = net_from_list(pattern, DT_COMPLEX, "ALGLIB: parameter 'pattern' of 'xalglib.corrc1dcircularbuf' must be complex vector")
    _net_n = n
    _net_c = net_from_list(c, DT_COMPLEX, "ALGLIB: parameter 'c' of 'xalglib.corrc1dcircularbuf' must be complex vector")
    try:

        # call function
        _net_c = _net_alglib.corrc1dcircularbuf(_net_signal, _net_m, _net_pattern, _net_n, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def corrr1dbuf(*functionargs):
    # unpack inputs
    signal, n, pattern, m, r = functionargs
    friendly_form = False

    # convert to .NET types
    _net_signal = net_from_list(signal, DT_REAL, "ALGLIB: parameter 'signal' of 'xalglib.corrr1dbuf' must be real vector")
    _net_n = n
    _net_pattern = net_from_list(pattern, DT_REAL, "ALGLIB: parameter 'pattern' of 'xalglib.corrr1dbuf' must be real vector")
    _net_m = m
    _net_r = net_from_list(r, DT_REAL, "ALGLIB: parameter 'r' of 'xalglib.corrr1dbuf' must be real vector")
    try:

        # call function
        _net_r = _net_alglib.corrr1dbuf(_net_signal, _net_n, _net_pattern, _net_m, _net_r)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def corrr1dcircularbuf(*functionargs):
    # unpack inputs
    signal, m, pattern, n, c = functionargs
    friendly_form = False

    # convert to .NET types
    _net_signal = net_from_list(signal, DT_REAL, "ALGLIB: parameter 'signal' of 'xalglib.corrr1dcircularbuf' must be real vector")
    _net_m = m
    _net_pattern = net_from_list(pattern, DT_REAL, "ALGLIB: parameter 'pattern' of 'xalglib.corrr1dcircularbuf' must be real vector")
    _net_n = n
    _net_c = net_from_list(c, DT_REAL, "ALGLIB: parameter 'c' of 'xalglib.corrr1dcircularbuf' must be real vector")
    try:

        # call function
        _net_c = _net_alglib.corrr1dcircularbuf(_net_signal, _net_m, _net_pattern, _net_n, _net_c)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def pcabuildbasis(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        x,  = functionargs
        npoints = check_equality_and_get([safe_rows("'pcabuildbasis': incorrect parameters",x)],"Error while calling 'pcabuildbasis': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'pcabuildbasis': incorrect parameters",x)],"Error while calling 'pcabuildbasis': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'pcabuildbasis': function must have 3 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_s2, _net_v = _net_alglib.pcabuildbasis(_net_x, _net_npoints, _net_nvars)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def pcatruncatedsubspace(*functionargs):
    ...
    if len(functionargs)==6:
        ...
        # short-form call
        x, nneeded, eps, maxits = functionargs
        npoints = check_equality_and_get([safe_rows("'pcatruncatedsubspace': incorrect parameters",x)],"Error while calling 'pcatruncatedsubspace': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'pcatruncatedsubspace': incorrect parameters",x)],"Error while calling 'pcatruncatedsubspace': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'pcatruncatedsubspace': function must have 6 or 4 parameters")
...
def net_to_modelerrors_inplace(x,r):
    r.relclserror = x.relclserror
    r.avgce = x.avgce
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
...
def net_to_ahcreport_inplace(x,r):
    r.terminationtype = x.terminationtype
    r.npoints = x.npoints
    r.p = list_from_net(x.p, DT_INT)
    r.z = listlist_from_net(x.z, DT_INT)
...
def net_to_kmeansreport_inplace(x,r):
    r.npoints = x.npoints
    r.nfeatures = x.nfeatures
    r.terminationtype = x.terminationtype
    r.iterationscount = x.iterationscount
...
def net_to_dfreport_inplace(x,r):
    r.relclserror = x.relclserror
    r.avgce = x.avgce
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
...
def net_to_lrreport_inplace(x,r):
    r.c = listlist_from_net(x.c, DT_REAL)
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
    r.avgrelerror = x.avgrelerror
...
def lrbuild(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        xy,  = functionargs
        npoints = check_equality_and_get([safe_rows("'lrbuild': incorrect parameters",xy)],"Error while calling 'lrbuild': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'lrbuild': incorrect parameters",xy)-1],"Error while calling 'lrbuild': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'lrbuild': function must have 3 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_lm, _net_rep = _net_alglib.lrbuild(_net_xy, _net_npoints, _net_nvars)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)

    # convert from .NET types
    lm = linearmodel(_net_lm)
    rep = lrreport_from_net(_net_rep)
...
def lrbuilds(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        xy, s = functionargs
        npoints = check_equality_and_get([safe_rows("'lrbuilds': incorrect parameters",xy), safe_len("'lrbuilds': incorrect parameters",s)],"Error while calling 'lrbuilds': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'lrbuilds': incorrect parameters",xy)-1],"Error while calling 'lrbuilds': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'lrbuilds': function must have 4 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_lm, _net_rep = _net_alglib.lrbuilds(_net_xy, _net_s, _net_npoints, _net_nvars)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lrbuildzs(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        xy, s = functionargs
        npoints = check_equality_and_get([safe_rows("'lrbuildzs': incorrect parameters",xy), safe_len("'lrbuildzs': incorrect parameters",s)],"Error while calling 'lrbuildzs': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'lrbuildzs': incorrect parameters",xy)-1],"Error while calling 'lrbuildzs': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'lrbuildzs': function must have 4 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_lm, _net_rep = _net_alglib.lrbuildzs(_net_xy, _net_s, _net_npoints, _net_nvars)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lrbuildz(*functionargs):
    ...
    if len(functionargs)==3:
        ...
        # short-form call
        xy,  = functionargs
        npoints = check_equality_and_get([safe_rows("'lrbuildz': incorrect parameters",xy)],"Error while calling 'lrbuildz': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'lrbuildz': incorrect parameters",xy)-1],"Error while calling 'lrbuildz': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'lrbuildz': function must have 3 or 1 parameters")
    ...
    try:
        ...
        # call function
        _net_lm, _net_rep = _net_alglib.lrbuildz(_net_xy, _net_npoints, _net_nvars)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def lrpack(*functionargs):
    ...
    if len(functionargs)==2:
        ...
        # short-form call
        v,  = functionargs
        nvars = check_equality_and_get([safe_len("'lrpack': incorrect parameters",v)+1],"Error while calling 'lrpack': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'lrpack': function must have 2 or 1 parameters")
...
def filtersma(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.filtersma(_net_x, _net_n, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def filterema(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.filterema(_net_x, _net_n, _net_alpha)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def filterlrma(*functionargs):
    ...
    try:
        ...
        # call function
        _net_alglib.filterlrma(_net_x, _net_n, _net_k)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fisherlda(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        xy, nclasses = functionargs
        npoints = check_equality_and_get([safe_rows("'fisherlda': incorrect parameters",xy)],"Error while calling 'fisherlda': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'fisherlda': incorrect parameters",xy)-1],"Error while calling 'fisherlda': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'fisherlda': function must have 4 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_w = _net_alglib.fisherlda(_net_xy, _net_npoints, _net_nvars, _net_nclasses)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def fisherldan(*functionargs):
    ...
    if len(functionargs)==4:
        ...
        # short-form call
        xy, nclasses = functionargs
        npoints = check_equality_and_get([safe_rows("'fisherldan': incorrect parameters",xy)],"Error while calling 'fisherldan': looks like one of arguments has wrong size")
        nvars = check_equality_and_get([safe_cols("'fisherldan': incorrect parameters",xy)-1],"Error while calling 'fisherldan': looks like one of arguments has wrong size")
        friendly_form = True
    else:
        raise RuntimeError("Error while calling 'fisherldan': function must have 4 or 2 parameters")
    ...
    try:
        ...
        # call function
        _net_w = _net_alglib.fisherldan(_net_xy, _net_npoints, _net_nvars, _net_nclasses)
    except _net_alglib.alglibexception as e:
        raise RuntimeError(e.msg)
...
def net_to_mcpdreport_inplace(x,r):
    r.inneriterationscount = x.inneriterationscount
    r.outeriterationscount = x.outeriterationscount
    r.nfev = x.nfev
    r.terminationtype = x.terminationtype
...
def net_to_mnlreport_inplace(x,r):
    r.ngrad = x.ngrad
    r.nhess = x.nhess
    return
...
def net_to_knnreport_inplace(x,r):
    r.relclserror = x.relclserror
    r.avgce = x.avgce
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
...
def net_to_mlpreport_inplace(x,r):
    r.relclserror = x.relclserror
    r.avgce = x.avgce
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
...
def net_to_mlpcvreport_inplace(x,r):
    r.relclserror = x.relclserror
    r.avgce = x.avgce
    r.rmserror = x.rmserror
    r.avgerror = x.avgerror
